...
set output 'student_t_plots_log.tex'
unset clip points
set clip one
unset clip two
...
logSt(x, mu, sigma, nu) = lgamma(0.5*(nu + 1)) - lgamma(0.5*nu) - 0.5*log(pi*nu*sigma**2) - 0.5*(nu + 1)*log(1 + (x - mu)**2/(nu*sigma**2))
GNUTERM = "x11"
...
plot [-15:15][-10:0] logN(x, 0, 1) title '$\nu \to \infty$', logSt(x, 0, 1, 1) title '$\nu = 1$', logSt(x, 0, 1, 0.1) title '$\nu = 0.1$'
#    EOF
