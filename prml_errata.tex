...
In addition to the mean~$\mathbb{E}[\lambda]$ and the variance~$\operatorname{var}[\lambda]$ of
the gamma distribution~$\operatorname{Gam}\left(\lambda\middle|a, b\right)$
given by (2.147) and (2.148), respectively,
we are also interested in its log expectation~$\mathbb{E}\left[ \ln\lambda \right]$
...
concerned in Exercise~2.11 (its mean and covariance are in Exercise~2.10).
Following a similar pattern,
we can calculate the log expectation of the gamma distribution~(2.146)
by making use of the derivative of its logarithm with respect to $a$,
which is given by
\begin{equation}
...
\end{equation}
where $\psi(\cdot)$ is the \emph{digamma function} defined by (B.25).

It is worth noting here that,
for any correctly normalized probability~$p\left(\mathbf{x}\middle|\bm{\theta}\right)$
parameterized by $\bm{\theta}$ and differentiable with respect to $\bm{\theta}$,
the expectation of its \emph{Fisher score}~(6.32),
...
\end{equation}
One can show the above identity~\eqref{eq:expected_fisher_score}
...
Now, substituting \eqref{eq:derivative_of_log_gamma_distribution_wrt_a} into
...
can be exactly evaluated in this case so that
\begin{equation}
\mathbb{E}\left[ \ln \lambda \right] = \ln \mathbb{E}\left[ \lambda \right] - \varphi(a)
\end{equation}
where $\varphi(a) > 0$ is the \emph{log minus digamma function} defined by
\begin{equation}
\varphi(a) \equiv \ln a - \psi(a) .
\label{eq:log_minus_digamma_function}
\end{equation}
The log minus digamma function~\eqref{eq:log_minus_digamma_function} also arises in
the maximum likelihood solutions for the gamma distribution~(2.146) as well as
in the EM algorithm for the Student's t-distribution~(2.159) or (2.162),
