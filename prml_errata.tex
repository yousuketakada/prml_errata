...
\end{equation}
respectively.
Note that (C.25) and (C.26) are special cases of \eqref{eq:gradient_of_Tr_AB_wrt_A}.

\parhead{Gradient of log determinant}
...
making use of (C.23), which can be stated more suitably for our purpose as
\begin{equation}
...
\end{equation}
where the Jacobian~$\partial\mathbf{A}/\partial A_{ij}$ is
a matrix of the same dimensionality as $\mathbf{A}$
such that only the $(ij)$-th element is one and all the other elements are zero.
...
\parhead{Some more matrix derivatives identities}
