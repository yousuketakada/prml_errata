...
it can be shown to be the marginal for a ``linearized'' version of the classification problem
...
As the distributions over $\mathbf{w}$ and $\bm{\mathsf{t}}$ are approximated as in
\eqref{eq:posterior_for_RVM_classification} and (7.114), respectively,
let us assume that the distribution over $\widehat{\bm{\mathsf{t}}}$
can also be approximated by the Laplace approximation.
Pretending that these Gaussian approximations are exact and
...
\end{equation}
The above marginal~\eqref{eq:marginal_for_linearized_RVM_classification} takes
