...
we can most easily calculate the log expectation of the gamma distribution.
...
over some random variables~$\mathbf{x}$
...
\nabla f = f \, \nabla \ln f .
\end{equation}
Although we have assumed here that $\mathbf{x}$ are continuous,
...
by substituting the derivative of the log probability
with respect to $b$ into \eqref{eq:expected_fisher_score}.
