...
\begin{equation}
\frac{\partial}{\partial x}\left(\mathbf{A}^{-1}\mathbf{B}\right)
= - \mathbf{A}^{-1} \frac{\partial \mathbf{A}}{\partial x} \mathbf{A}^{-1} \mathbf{B}
\end{equation}
which can be obtained by differentiating the both sides of
the identity~$\mathbf{A}\mathbf{A}^{-1}\mathbf{B} = \mathbf{B}$
or by right-multiplying $\mathbf{B}$ to the both sides of (C.21).
