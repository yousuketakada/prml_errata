...
that for the parameters of the Wishart distribution is not given here nor in Exercise~2.45.
...
the Wishart prior~\eqref{eq:wishart_prior}, namely
the scale matrix~$\mathbf{W}_0$ and the number of degrees of freedom~$\nu_0$.
Since observing $N$ data points increases the number of degrees of freedom by $N$,
we can interpret $\nu_0$ as the number of ``effective'' prior observations.
The $N$ observations also contribute $N\bm{\Sigma}_{\text{ML}}$ to the inverse of the scale matrix
where $\bm{\Sigma}_{\text{ML}}$ is the maximum likelihood estimate for the covariance given by
\begin{equation}
\bm{\Sigma}_{\text{ML}} = \frac{1}{N} \sum_{n=1}^{N}\mathbf{x}_n\mathbf{x}_n^{\operatorname{T}}
\end{equation}
which suggests an interpretation of $\mathbf{W}_0$ in terms of
$\bm{\Sigma}_0 \equiv \left(\nu_0\mathbf{W}_0\right)^{-1}$, that is,
$\bm{\Sigma}_0$ is the covariance of the prior observations.
Note that this interpretation is in accordance with the observation that, form (B.80),
