...
Suppose that, given two (grayscale) images $I_0(\mathbf{x})$ and $I_1(\mathbf{x})$ taken at
...
The \emph{Horn-Schunck optical flow}~\citep{HornSchunck:OpticalFlow} seeks for
$\mathbf{u}(\mathbf{x})$ that minimizes an energy functional of the form
\begin{equation}
J[\mathbf{u}(\mathbf{x})] =
...
\right) \mathrm{d}\mathbf{x} .
\end{align}
TODO: Derive the Euler-Lagrange equations~\eqref{eq:euler_lagrange_equation} for
the optical-flow functional~\eqref{eq:horn_schunck_optical_flow_functional}.
