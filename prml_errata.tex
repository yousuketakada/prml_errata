...
implying that it is a generalization of the Bernoulli distribution~(B.1)
...
where $N = 1$, i.e., for a single observation
...
$A \Rightarrow B$ denotes
``$A$ generalizes to $B$ with multiple categories concerned''; and
$A \rightarrow B$ ``$A$ to $B$ with multiple observations.''
$A \dashrightarrow B$ denotes ``$A$ is the conjugate prior for $B$.''}
