...
so is \eqref{eq:alpha_in_cholesky_factor} because $a - \|\bm{\beta}\|^{2} > 0$,
...
because the elements in the Cholesky factor~$\mathbf{U} = (U_{ij}) \in \mathcal{U}_{+}^{D}$ have
a simpler domain of integration,
i.e., $U_{ii} \in (0, \infty)$ and $U_{ij} \in (-\infty, \infty)$ where $i < j$.
Here, we consider some necessary Jacobians for such matrix transformations.
...
\end{align}
Since $A_{ij}$ depends only on $U_{ij}$ and its preceding elements in lexicographic order,
...
Another matrix transformation we consider here is a linear transformation between
...
\label{eq:element_wize_product_of_upper_triangular_matrix} \\
\shortvdotswithin{=}
U_{DD} &= R_{DD} G_{DD} .
\end{align}
Since $U_{ij}$ depends only on $R_{ij}$ and its preceding elements in lexicographic order,
...
then the log determinant expectation~(B.81) can be written as
\begin{equation}
\mathbb{E}\left[ \ln \left| \bm{\Lambda} \right| \right]
= \psi_D\left(\frac{\nu}{2}\right) + D \ln 2 + \ln \left| \mathbf{W} \right| .
...
\begin{equation}
\mathrm{d}\mathbf{X} \equiv \prod_{i \leqslant j} \mathrm{d}X_{ij}
...
&= 2^{D}
\left[
\prod_{i} \int_{0}^{\infty} U_{ii}^{2a - i} \exp\left( -U_{ii}^{2} \right) \mathrm{d}U_{ii}
\right]
\left[
\prod_{i < j} \int_{-\infty}^{\infty} \exp\left( -U_{ij}^2 \right) \mathrm{d}U_{ij}
\right]
\\
&= \pi^{D(D - 1)/4} \prod_{i=1}^{D} \Gamma\left(a - \frac{i - 1}{2}\right)
\end{align}
...
= \prod_{i} U_{ii}^2 \\
\operatorname{Tr}\left( \mathbf{X} \right) &= \sum_{i} X_{ii}
= \sum_{i} \left( U_{1i}^2 + \dots + U_{ii}^2 \right)
= \sum_{i \leqslant j} U_{ij}^2
\end{align}
...
\parhead{Normalization constant of Wishart}
...
together with identities regarding the diagonal elements of
$\mathbf{U} = \left(U_{ij}\right),
\mathbf{R} = \left(R_{ij}\right),
\mathbf{G} = \left(G_{ij}\right)
\in \mathcal{U}_{+}^{D}$,
that is,
$U_{ii} = R_{ii} G_{ii}$, which follows from
\eqref{eq:element_wize_product_of_upper_triangular_matrix}, and
$\left|\mathbf{G}\right| = \prod_{i} G_{ii}$.

Evaluating the right hand side of
\eqref{eq:integral_identity_for_normalization_constant_of_wishart}
by making the changes of variables~\eqref{eq:changes_of_variables_for_wishart_normalization} and
noting the identities
\begin{align}
\left| \bm{\Lambda} \right|
&= \left| \mathbf{U}^{\operatorname{T}} \mathbf{U} \right|
= \left| \mathbf{G} \right|^{2} \left| \mathbf{R}^{\operatorname{T}} \mathbf{R} \right| \\
\frac{1}{2} \operatorname{Tr}\left( \mathbf{W}^{-1} \bm{\Lambda} \right)
&= \operatorname{Tr}\left(
\left(
\mathbf{G}^{\operatorname{T}} \mathbf{G}
\right)^{-1}
\mathbf{U}^{\operatorname{T}} \mathbf{U}
\right)
= \operatorname{Tr}\left(
\mathbf{R}^{\operatorname{T}} \mathbf{R}
\right)
\end{align}
we have
\begin{align}
...
\end{align}
where we have used
the relation~$\left|\mathbf{G}\right| = 2^{D/2} \left| \mathbf{W} \right|^{1/2}$
