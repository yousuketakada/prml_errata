...
For a probability distribution~$p\left(\mathbf{x}\middle|\bm{\theta}\right)$
over random variables~$\mathbf{x}$ parameterized by $\bm{\theta}$
such that $p\left(\mathbf{x}\middle|\bm{\theta}\right)$ is logarithmically differentiable
with respect to the parameters~$\bm{\theta}$ and
correctly normalized under sufficiently small perturbations in $\bm{\theta}$,
