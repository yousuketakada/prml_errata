...
the singular value decomposition~$\mathbf{A} = \mathbf{U}\bm{\Sigma}\mathbf{V}^{\operatorname{T}}$
...
in which case it is no longer possible to take the orthonormal matrix~$\mathbf{U}$ such that
$\operatorname{det}(\mathbf{U}) = 1$ in general.
