...
\leq \mathbb{E}_{\mathbf{z}}\left[f\left(\xi(\mathbf{z})\right)\right]
...
\leq \mathbb{E}\left[f\left(\xi\right)\right]
...
\begin{equation}
f\left(\int\xi(\mathbf{x}) p(\mathbf{x}) \,\mathrm{d}\mathbf{x}\right) \leq 
...
$\operatorname{KL}\left( p \middle\| q \right) \geq 0$ with equality if and only if
