...
The vector derivative of this form~(C.17) is called the \emph{gradient}
whereas (C.18) is the \emph{Jacobian}~\citep{Minka:OldNewMatrixAlgebra}.
Thus, we should use a different notation, say, $\nabla$ for the gradient to avoid ambiguity.
...
\end{equation}
Note that the gradient is identical to the transpose of the Jacobian
matrix~$\partial\mathbf{y}/\partial\mathbf{x} =
\left( \partial y_i/\partial x_j \right)$
so that $\nabla_{\mathbf{x}} \mathbf{y} =
\left( \partial\mathbf{y}/\partial\mathbf{x} \right)^{\operatorname{T}}$,
as a consequence of which the chain rule for the gradient is built up ``towards the left,'' i.e.,
\begin{equation}
...
as in, e.g., \eqref{eq:gradient_of_linear_regression_likelihood},
I would suggest that it should also be pointed out in the ``Matrix Derivatives'' section of
Appendix~C.
...
\nabla \left\{ \mathbf{A}\mathbf{x} \right\} & = \mathbf{A}^{\operatorname{T}} \\
\nabla \left\{ \phi\mathbf{y} \right\} &=
\nabla\phi \mathbf{y}^{\operatorname{T}} + \phi \nabla\mathbf{y} .
\end{align}
Note that $\mathbf{x}^{\operatorname{T}} \mathbf{A} \mathbf{x}$ is a quadratic form and thus
...
(up to the second order) of a scalar function $f(\mathbf{x})$ in terms of the gradients as
\begin{equation}
