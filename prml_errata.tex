...
one might wonder why we use two different forms of a vector derivative that are identical
...
it is necessary that we have $\operatorname{det}(\mathbf{A}) > 0$.\footnote{This is
implied by $\mathbf{A} \succ 0$ as we usually assume.}.
Or, if we adopt the notation~\eqref{eq:abs_of_matrix_determinant} for the absolute value of
a matrix determinant, (C.22) holds for any non-singular $\mathbf{A}$.
