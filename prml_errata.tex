...
implies that $\xi(\mathbf{z})$ is constant almost surely
...
$0 \leqslant \mu_k \leqslant 1$ and $\sum_k \mu_k = 1$.\footnote{%
Strictly speaking, we cannot take the logarithm~$\ln \mu_k$ for $\mu_k = 0$
in the right hand side of \eqref{eq:multinoulli_distribution}.
However, in the context of Bayesian inference in which we regard $\bm{\mu}$ as random variables,
this will not be a problem because we have $0 < \mu_k < 1$ with probability one
(unless the distribution is singular).}
