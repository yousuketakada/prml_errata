...
That is because Jacobians are useful in calculus while gradients are useful in optimization
\citep{Minka:OldNewMatrixAlgebra}.}
...
there is no point to do so in practice.
In a special case of $\mathbf{A}$ being positive semidefinite, 
we can practically identify the eigenvalue decomposition~(C.43) with
the singular value decomposition and the eigenvalues with the singular values.
The singular values are all nonnegative and usually arranged in descending order,
in which case it is no longer possible to take $\operatorname{det}(\mathbf{U}) = 1$ in general.

\erratum{Page~700}
The text following (C.41):
The multiplication by $\mathbf{U}$ can be a rotation, a reflection, or a combination of them.
