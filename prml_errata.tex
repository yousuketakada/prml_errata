...
\end{align}
where we have used the simplified form~%
\eqref{eq:wishart_normalization_constant_in_terms_of_multivariate_gamma_function}
of the normalization constant of the Wishart distribution
in terms of the multivariate gamma function~%
...
\end{align}
Since $A_{ij}$ depends only on $U_{ij}$ and the preceding elements in lexicographic order,
...
U_{DD} &= R_{DD} G_{DD} .
\end{align}
Since $U_{ij}$ depends only on $R_{ij}$ and the preceding elements in lexicographic order,
the Jacobian matrix from $\mathbf{R}$ to $\mathbf{U}$ is again a lower triangular matrix
such that the diagonal elements are given by
\begin{equation}
\frac{\partial U_{ij}}{\partial R_{ij}} = G_{jj} > 0
\end{equation}
where $i \leqslant j$.
Thus, we obtain the Jacobian in the form
\begin{equation}
...
The multivariate gamma function~\citep{Anderson:Multivariate,NIST:DLMF} is defined by
\begin{equation}
...
\end{equation}
where the integration is taken over the space~$\mathcal{S}_{+}^{D}$ of
...
the (univariate) gamma function~$\Gamma(\cdot)$ given by (1.141).

The multivariate gamma function~\eqref{eq:definition_of_multivariate_gamma_function}
can also be written in terms of the (univariate) gamma function as
\begin{equation}
\Gamma_D \left(\frac{\nu}{2}\right) =
...
so that we can simplify the normalization constant~(B.79) of the Wishart distribution in the form
\begin{equation}
B(\mathbf{W}, \nu)^{-1} =
2^{\nu D/2}
\left|\mathbf{W}\right|^{\nu/2}
