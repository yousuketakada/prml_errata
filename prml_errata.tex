...
Here, an upper triangular matrix~$\mathbf{T} = \left(T_{ij}\right)$
is a square matrix such that $T_{ij} \equiv 0$ where $i > j$ so that
\begin{equation}
\mathbf{T} =
\begin{pmatrix}
...
\end{pmatrix}
\end{equation}
where $D$ is the dimensionality of $\mathbf{T}$.
Similarly, a square matrix~$\mathbf{L}$ such that $\mathbf{L}^{\operatorname{T}}$ is
upper triangular is called a \emph{lower triangular} matrix.

If $\mathbf{T} \in \mathcal{T}_{+}^{D}$,
then $\mathbf{T} = \left(T_{ij}\right)$ is an upper triangular matrix of dimensionality~$D$
such that $T_{ii} > 0$ for all $i$.
...
for $\mathbf{T} \in \mathcal{T}_{+}^{D}$, consider its \emph{characteristic equation}~(C.30),
from which it readily follows that the eigenvalues of $\mathbf{T} = \left(T_{ij}\right)$ are
equal to its diagonal elements~$T_{ii} > 0$.
