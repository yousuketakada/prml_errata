...
Moreover, it should also be noted that the limit can be context dependent, that is,
the big O notation~$g(x) = O\left(f(x)\right)$ generally denotes that
$\left|g(x)/f(x)\right|$ is bounded as $x \to c$
where, if $c$ is not explicitly stated,
$c = 0$ for a Taylor series such as (2.299) and (D.1); or
$c = \infty$
for computational complexity (see, e.g., Section~5.2.3) or
for an asymptotic series~(10.241), for example.
