...
Of course, nothing prevents us from examining the behavior of both the two criteria, if possible,
for assessing the model concerned.
If both the criteria indicate that a model is favorable or not,
then it is probably safe to say that the model is sensible or not, respectively.
At least it is hoped that, since the two criteria are different,
