...
\erratum{Page~687}
Equation~(B.20):
The mode~(B.20) of the Dirichlet exists if $\alpha_i > 1$ for all $i$.
...
so that we can use an SVD routine to compute the eigenvalue decomposition.
The SVD is generally defined for any real matrix~$\mathbf{A}$ not necessarily square,
say, of dimensionality~$M \times N$,
so that the SVD of $\mathbf{A}$ is given by
\begin{equation}
\mathbf{A} = \mathbf{U}\bm{\Sigma}\mathbf{V}^{\operatorname{T}}
\end{equation}
where $\mathbf{U}$ and $\mathbf{V}$ are orthogonal matrices of
dimensionality~$M \times M$ and $N \times N$, respectively;
and $\bm{\Sigma}$ is an $M \times N$ diagonal matrix with
nonnegative diagonal elements, called the \emph{singular values},
such that $\sigma_1 \geqslant \sigma_2 \geqslant \dots \geqslant 0$.
