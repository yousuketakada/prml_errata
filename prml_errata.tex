...
\end{align}
for any $\mathcal{X}_0 = g(\mathcal{Y}_0)$
or $\mathcal{Y}_0 = g^{-1}(\mathcal{X}_0)$
where $\mathcal{X}_0 \subset \mathcal{X}$
and $\mathcal{Y}_0 \subset \mathcal{Y}$,
then we can identify the integrand of the right hand side as $p_y(y)$.

\parhead{Multivariate change of variables}
Similarly, the multivariate version of the change of variables formula~(1.27) is given by
\begin{equation}
p(\mathbf{y}) =
p(\mathbf{x})
\left|
...
% Due to Mark-Jan Nederhof
Equation~(11.6):
The transformation~$f: (0, 1) \to (-\infty, \infty)$ between the variables~$z$ and $y$,
which is of course \emph{bijective}
as we have assumed for the general change of variables formula~%
\eqref{eq:multivariate_change_of_variables},
is also assumed to be \emph{monotonically increasing} in (11.6) so that
\begin{equation}
p\left(y \in (-\infty, y_0)\right) =
p\left(z \in (0, z_0)\right)
\end{equation}
where $y_0 = f(z_0)$.
...
a \emph{bijective} function~$\mathcal{S}_{+}^{D} \to \mathcal{U}_{+}^{D}$.
