...
\erratum{Page~93}
Equation~(2.120):
The gradient operator~$\frac{\partial}{\partial \bm{\mu}}$ should read $\nabla_{\bm{\mu}}$
if we adopt the definition of the gradient operator~$\nabla$
given by \eqref{eq:gradient_of_vector_wrt_vector}.
...
the gradient operator~$\nabla$ used here has not been properly defined yet.
...
Moreover, the definitions for $\frac{\partial}{\partial \mathbf{x}}$ given in Appendix~C
are fragile and inappropriate (we shall come back to this issue later in this report).
...
\end{equation}
where we have written the variable~$\mathbf{w}$ to which we take the gradient
in the subscript of $\nabla$ explicitly.
See \eqref{eq:gradient_of_vector_wrt_vector} for the definition of the gradient operator~$\nabla$
adopted in this report.
