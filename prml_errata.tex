...
The \emph{image} of a subset~$\mathcal{X}_0 \subset \mathcal{X}$ of the domain~$\mathcal{X}$
under $f$ is a subset of the codomain~$\mathcal{Y}$ denoted by
$f(\mathcal{X}_0) \subset \mathcal{Y}$ so that
$f(\mathcal{X}_0) \equiv
\{ y \in \mathcal{Y} \mid \text{$y = f(x) $ for some $ x \in \mathcal{X}_0 $}\}$.}
...
Here, the proviso~\emph{almost surely} (a.s.) or \emph{almost everywhere} (a.e.) means that
...
it is a concept formally defined in a branch of mathematics called \emph{measure theory},
...
this property is called \emph{zero-forcing} (see also Section~10.1.2).\footnote{%
The zero-forcing condition can be understood intuitively
from the perspective of information theory as follows.
As we have seen in Section~1.6.1,
the Kullback-Leibler divergence~$\operatorname{KL}\left( p \middle\| q \right)$
can be interpreted as
the average information wasted to encode samples generated from the source~$p$
...
\end{align}
where $0 < \lambda < 1$.
Here, we again assume that the zero-forcing condition holds
for each pair of the distributions~$(p_i, q_i)$
so that $p_i(\mathbf{x}) = 0$
outside the support~$\Omega_i = \{ \mathbf{x} \mid q_i(\mathbf{x}) > 0 \}$ of $q_i(\mathbf{x})$
where $i = 1, 2$.
Writing
\begin{align}
\begin{aligned}
...
the parameter~$0 \leqslant \mu \leqslant 1$ (Page~69, Paragraph~1, Line~1),
it becomes \emph{degenerate} if $\mu = 0$ or $\mu = 1$, i.e.,
$x$ is fixed to a single value so that $x = 0$ or $x = 1$, respectively.\footnote{%
Note however that, in the context of Bayesian inference
in which we regard the parameter~$\mu$ as a random variable,
the distribution will not be degenerate because we have $0 < \mu < 1$ almost surely
...
so that we can write\footnote{%
Yet, we can still consider the degenerate case as the limit~$\mu \to 0$ or $\mu \to 1$.}
\begin{equation}
...
See also the \emph{multinoulli} distribution~\eqref{eq:multinoulli_distribution}.

Accordingly, we also assume similar restrictions $\mu \in (0, 1)$ and $\mu_k \in (0, 1)$
on the domains of the (conjugate) prior distributions, i.e.,
the \emph{beta} distribution~(2.13) and the \emph{Dirichlet} distribution~(2.38)
so that we can avoid potential difficulty that
the density function diverges at the boundary of the domain (see Figures~2.2 and 2.5)
while this does not affect their correct normalization.
...
\erratum{Page~147}
Paragraph~\textminus2:
The argument that ``the phenomenon of [overfitting] does not arise
