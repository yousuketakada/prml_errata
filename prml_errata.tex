...
As \citet{Svensen:PRML_errata} corrects the left hand side of (10.12),
we should write, e.g., $q_i(z_i)$ instead of $q(z_i)$ in the text.
Or, we should clarify that the variational distribution over some latent variables $\mathbf{z}_i$
is denoted by $q(\mathbf{z}_i)$
in the same manner as the probability~$p(\cdot)$ is ``overloaded'' by its arguments.
