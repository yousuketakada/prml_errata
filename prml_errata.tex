...
The following gives an interpretation for the parameters of the Wishart distribution.
...
\erratum{Page~355}
Equation~(7.118):
This approximate marginal distribution is different from the Laplace approximation~(7.114) to
the marginal~$p\left(\bm{\mathsf{t}}\middle|\bm{\alpha}\right)$;
it \emph{is} the marginal for a ``linearized'' version of the RVM classification problem
...
We have already seen that the posterior can be approximated by the Laplace approximation as
\begin{equation}
...
Assuming that the distribution over $\widehat{\bm{\mathsf{t}}}$ as well as $\mathbf{w}$
can be approximated by the Laplace approximation;
and making use of the (approximate) linear-Gaussian relation,
we can obtain the corresponding marginal for this linearized problem as
\begin{equation}
...
\end{equation}
The above marginal~\eqref{eq:marginal_for_linearized_RVM_classification} (approximately) takes
