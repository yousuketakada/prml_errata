...
We have obtained the maximum likelihood solutions~$\bm{\mu}_\text{ML}$ and $\bm{\Sigma}_\text{ML}$
for the Gaussian by setting the derivatives of
the log likelihood~$\ln p\left(\mathbf{X}\middle| \bm{\mu}, \bm{\Sigma}\right)$ given by (2.118)
with respect to $\bm{\mu}$ and $\bm{\Sigma}$ equal to zero,
...
This can be easily done for $\bm{\mu}_\text{ML}$ by first noting that
the log likelihood~(2.118) is quadratic in $\bm{\mu}$, i.e.,
\begin{equation}
\ln p\left(\mathbf{X}\middle| \bm{\mu}, \bm{\Sigma}\right) =
-\frac{N}{2} \bm{\mu}^{\operatorname{T}} \bm{\Sigma}^{-1} \bm{\mu}
+ \bm{\mu} \bm{\Sigma}^{-1} \left( \sum_{n=1}^{N} \mathbf{x}_n \right)
+ \text{const} .
\end{equation}
Since the covariance~$\bm{\Sigma}$ is positive definite and so is its inverse~$\bm{\Sigma}^{-1}$,
we see that the log likelihood is concave with respect to $\bm{\mu}$ and, thus,
$\bm{\mu}_\text{ML}$ indeed maximizes the likelihood.
In order to show that $\bm{\Sigma}_\text{ML}$ maximizes the likelihood
with respect to $\bm{\Sigma}$,
we consider the eigenvalue expansion~(2.48) for $\bm{\Sigma}$ so that we have
\begin{equation}
\ln p\left(\mathbf{X}\middle| \bm{\mu}, \bm{\Sigma}\right) =
-\frac{N}{2} \sum_{i=1}^{D} \ln \lambda_i
-\frac{1}{2} \sum_{n=1}^{N} \sum_{i=1}^{D} \frac{y_{ni}}{\lambda_i}
+ \text{const}
...
then $\bm{\Sigma}_\text{ML}$ maximizes the likelihood.
