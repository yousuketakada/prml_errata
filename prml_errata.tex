...
Here, we have defined $0^0 \equiv 1$ (similarly to $0 \ln 0 \equiv 0$).}
...
the distribution parameterized by $\mu$ cannot be degenerate
because we have $0 < \mu < 1$ almost surely
...
for which we assume the restrictions~$\mu \in (0, 1)$ and $\mu_k \in (0, 1)$, respectively.
See also our definition~\eqref{eq:multinoulli_distribution} of the \emph{multinoulli} distribution.

Accordingly, the same restrictions~$\mu \in (0, 1)$ and $\mu_k \in (0, 1)$ are assumed also on
the domains of the (conjugate) prior distributions, i.e.,
...
We assume in this report that $\mu_k \in (0, 1)$.
See \eqref{eq:Bernoulli_in_exponential_of_logarithm}.
...
\erratum{Page~686}
Paragraph~1, Line~\textminus3:
Since we assume in this report that $\mu \in (0, 1)$, there is no singularity.
See \eqref{eq:Bernoulli_in_exponential_of_logarithm}.
...
\erratum{Page~687}
Paragraph~\textminus1, Line~\textminus1:
Since we assume in this report that $\mu_k \in (0, 1)$, there is no singularity
as with the beta distribution.
See \eqref{eq:Bernoulli_in_exponential_of_logarithm}.
...
where $a < b$ should be $x \in (a, b)$ for consistency.
