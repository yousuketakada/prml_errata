...
\erratum{Page~56}
The first sentence after (1.114):
Since a convex function is not necessarily differentiable
(consider, e.g., the absolute value function~$f(x) = |x|$, which is not differentiable but convex),
we should restrict the class of the function considered here
in order to make the statement valid.
Moreover, strict positivity of the second derivative is also not necessary for convexity.
Specifically, the first sentence after (1.114) should read, e.g.,
``If the function~$f(x)$ is twice differentiable,
then the inequality condition~(1.114) for any pair of $a$ and $b$ is equivalent to
