...
$\left|f(x)/g(x)\right|$ should read $\left|g(x)/f(x)\right|$ (with the functions swapped) so that
the \emph{big O notation}~$g(x) = O\left(f(x)\right)$ denotes that
$\left|g(x)/f(x)\right|$ is bounded as $x \to \infty$.
With the swapped definition in PRML,
we would have, say, $x^2 = O(x)$
because $\left|x/x^2\right| \to 0$,
...
The function~$f(x)$ is a strictly convex function over $\left[0, \infty\right)$
where we have defined $0 \ln 0 = 0$;
it takes the minimum at $\left(e^{-1}, -e^{-1}\right)$.
The roots ($x$ such that $f(x) = 0$) are $x = 0$ and $x = 1$.}
\label{fig:x_ln_x}
\end{figure}
