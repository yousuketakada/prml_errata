...
backref=page,colorlinks,allcolors=MidnightBlue]{hyperref}
...
in terms of the variational parameters~$\bm{\xi}$ and thus not dependent on
the variational distribution~$q(\mathbf{w})$ explicitly.
...
It is also knowns as the \emph{multinoulli} distribution
in an analogy to the Bernoulli/binomial distribution,
implying that the multinoulli is a generalization of the Bernoulli distribution~(B.1)
to the $K$-dimensional binary variable~$\mathbf{x} = (x_1, \dots, x_K)^{\operatorname{T}}$
where $x_k \in \{0, 1\}$ such that $\sum_k x_k = 1$, i.e.,
we use the one-of-$K$ coding scheme for $\mathbf{x}$; and
it is also a special case of the multinomial distribution~(B.59)
where $N = 1$, i.e., for a single observation.\footnote{%
The multinoulli is to the multinomial what the Bernoulli is to the binomial;
the multinoulli is to the Bernoulli what the multinomial is to the binomial.
The relationship between the discrete distributions can be expressed as follows.
\begin{equation}
\xymatrix{
...
\end{equation}
where $A \Longrightarrow B$ denotes
``$A$ generalizes to $B$ with multiple categories concerned''; and
$A \longrightarrow B$ ``$A$ to $B$ with multiple observations.''
Note also that the conjugate prior for the Bernoulli and the binomial is the beta~(B.6);
and that for the multinoulli and the multinomial is the Dirichlet~(B.16).}
...
the distribution~\eqref{eq:multinoulli_distribution} can also be expressed
