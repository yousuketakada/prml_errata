...
\end{equation}
This suggests an interpretation of $\mathbf{W}$ in terms of the corresponding covariance matrix
given by
\begin{equation}
\bm{\Sigma} \equiv \left(\nu\mathbf{W}\right)^{-1}.
\end{equation}
More specifically,
$\bm{\Sigma}_0 = \left(\nu_0\mathbf{W}_0\right)^{-1}$ in the prior~\eqref{eq:wishart_prior} is
...
we need a more general proof where $\mathbf{A}$ is only assumed to be nonsingular.
