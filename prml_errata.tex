...
The change of variables based on the Cholesky decomposition is
useful for evaluating integrals over symmetric positive-definite matrices
because the elements in the Cholesky factor~$\mathbf{U} = (U_{ij})$ have
a simpler domain of integration,
i.e., $U_{ii} \in (0, \infty)$ and $U_{ij} \in (-\infty, \infty)$ where $i < j$.
We shall later see some examples of this technique.
...
\end{align}
Since $A_{ij}$ depends only on $U_{ij}$ and its preceding elements in $\mathbf{U}$,
...
Another matrix transformation of interest is a linear transformation between
$\mathbf{U} \in \mathcal{U}_{+}^{D}$ and $\mathbf{R} \in \mathcal{U}_{+}^{D}$
of the form
\begin{equation}
...
U_{DD} &= R_{DD} G_{DD} .
\end{align}
Since $U_{ij}$ depends only on $R_{ij}$ and its preceding elements in $\mathbf{R}$,
