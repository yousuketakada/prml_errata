...
\author{Yousuke Takada \\ \email{yousuketakada@gmail.com} \\ \twitter{Yousuke\_Takada}}
\date{\today}
\maketitle
...
We are also interested in the expectation of $\ln\lambda$ given by (B.30),
which can be easily shown by making use of the derivative of the gamma distribution~(2.146)
...
\begin{equation}
\int \operatorname{Gam}\left(\lambda\middle|a, b\right) \mathrm{d}\lambda = 1
\end{equation}
with respect to $a$,
...
\end{equation}
We shall make use of the log minus digamma function~$\varphi(\cdot)$ later in this report.
...
the variational parameters~$\bm{\xi}$ can be optimized by maximizing~(10.160).
This EM-like algorithm is not the same as the one we see in Chapter~9;
it can be derived again by maximizing the lower bound~(10.3).
...
\end{equation}
where we bound the likelihood~$p\left(\mathbf{X} \middle| \mathbf{Z}\right) \geqslant
...
Then, we can ``bound'' the lower bound~(10.3) as
\begin{equation}
...
With much the same discussion as for the standard variational Bayesian method where we assume
factorization~(10.5) of $q(\mathbf{Z})$,
the optimal solution for $q_j(\mathbf{Z}_j)$ that maximizes the right hand side of
\eqref{eq:bound_of_lower_bound} can be obtained by
\begin{equation}
\ln q_j^{\star}\left(\mathbf{Z}_j\right) =
\mathbb{E}_{\mathbf{Z}\setminus\mathbf{Z}_j}
\left[ \ln \widetilde{p}\left(\mathbf{X}, \mathbf{Z}; \bm{\xi}^{\text{old}}\right) \right] +
\text{const}
\label{eq:eq:optimal_solution_for_local_variational_bayes}
\end{equation}
where we have fixed the variational parameters to $\bm{\xi}^{\text{old}}$.
Note that the result~\eqref{eq:eq:optimal_solution_for_local_variational_bayes}
is analogous to \eqref{eq:optimal_solution_for_variational_bayes}.
The optimization of $\bm{\xi}$ can be done by maximizing the first term of the right hand side of
\eqref{eq:bound_of_lower_bound}, i.e.,
\begin{equation}
\mathcal{Q}\left(\bm{\xi}, \bm{\xi}^{\text{old}}\right) =
\mathbb{E}_{\mathbf{Z}}\left[\ln \widetilde{p}\left(\mathbf{X}, \mathbf{Z}; \bm{\xi}\right)\right].
\end{equation}
