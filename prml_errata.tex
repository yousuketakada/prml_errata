...
See also \citet{NIST:DLMF} and note that the big O notation is not necessarily ``tightest''
so that, e.g., we can write $g(x) = O(x^2)$ for $g(x) = ax^2 + bx + c$
where the coefficient~$a$ for $x^2$ can be either nonzero or zero
($a \neq 0$ or $a = 0$).
