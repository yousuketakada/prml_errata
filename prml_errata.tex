...
The expectations are notationally inconsistent with the one~(1.36) employed in PRML;
they should read $\mathbb{E}_{\mathbf{Z}}[\cdot]$ where $\mathbf{Z}$ is replaced with
the corresponding latent variable(s).
For example,
the expectations~$\mathbb{E}_{\alpha}\left[ \ln q(\mathbf{w}) \right]_{\mathbf{w}}$ in (10.107) and
$\mathbb{E}\left[\ln p\left(\bm{\mathsf{t}}\middle|\mathbf{w}\right)\right]_{\mathbf{w}}$
in (10.108) should read
...
the expectations~$\mathbb{E}_{\mathbf{Z}}[\cdot]$ here
because the variables over which we take the expectations are clear.
See, e.g., (10.70) and (10.92).
...
if $\nabla_{\mathbf{A}} \varphi(\mathbf{S})$ is symmetric
(which does hold, say,
