...
``$\mathbf{A} \succ 0$ is a symmetric positive-definite matrix of dimensionality~$D$.''
...
To see the first part, we use mathematical induction.
...
Similarly, the second part can also easily be shown by induction.
...
$\widehat{\mathbf{L}} \in \mathcal{T}_{+}^{D - 1}$ is again in
the group~$\mathcal{T}_{+}^{D - 1}$ so that
$\widehat{\mathbf{L}}^{-1} \in \mathcal{T}_{+}^{D - 1}$,
...
Note however that $\mathcal{S}_{+}^{D}$ does not form a group.
In fact, the product~$\mathbf{P} = \mathbf{A}\mathbf{B}$ of
$\mathbf{A}, \mathbf{B} \in \mathcal{S}_{+}^{D}$ is not symmetric in general.\footnote{%
Furthermore, we do \emph{not} in general have
$\mathbf{x}^{\operatorname{T}} \mathbf{P} \mathbf{x} > 0$
for $\mathbf{x} \neq \mathbf{0}$.
...
\end{equation}
The product~$\mathbf{P} = \mathbf{A}\mathbf{B}$ is given by
\begin{equation}
