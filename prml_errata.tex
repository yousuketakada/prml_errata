...
\end{align}
Thus, we see that $\operatorname{KL}\left(p\middle\|q\right)$ is convex with respect to $(p, q)$
so that
\begin{equation}
\lambda \operatorname{KL}\left( p_1 \middle\| q_1 \right) +
(1 - \lambda) \operatorname{KL}\left( p_2 \middle\| q_2 \right)
\geqslant \operatorname{KL}\left(
\lambda p_1 + (1 - \lambda) p_2 \middle\|
\lambda q_1 + (1 - \lambda) q_2
\right) ,
\quad 0 < \lambda < 1 .
\label{eq:strict_convexity_of_KL_wrt_pair_of_p_and_q}
\end{equation}
Note however that the convexity~\eqref{eq:strict_convexity_of_KL_wrt_pair_of_p_and_q}
is not strict in general
(consider, e.g., the case where $\Omega_1 \cap \Omega_2 = \emptyset$).

We also see that
the convexity~\eqref{eq:strict_convexity_of_KL_wrt_pair_of_p_and_q} with respect to $(p, q)$
implies that with respect to $p$,
which, in fact, can be shown to be strict with a similar derivation as above, i.e.,
we have that $\operatorname{KL}\left(p\middle\|q\right)$ is strictly convex with respect to $p$
so that
\begin{equation}
