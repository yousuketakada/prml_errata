...
\emph{convex} and \emph{strict convex} functions.
First, a convex function is not necessarily differentiable
(consider, e.g., the absolute value function~$f(x) = |x|$, which is not differentiable but convex).
Second, even for twice differentiable functions,
strict positivity of the second derivative is not necessary for convexity nor for strict convexity.
...
convex and strict convex functions
...
I also give some useful properties for convex and strict convex functions that are differentiable.
...
because convexity implies continuity (except on the boundary~$\partial\mathcal{X}$ if any).
To see this, consider three points~$A, B, C$ on the convex curve~$y = f(x)$
whose $x$ values are $x_0, x_1, x_2 \in \mathcal{X}$, respectively, where $x_0 < x_1 < x_2$.
First, we see from convexity that $B$ lies below the line~$AC$.
Again from convexity, we have that:
(i)~on the interval~$[x_0, x_1]$, the curve must be below $AB$ and above $BC$; and
(ii)~on $[x_1, x_2]$, the curve must be below $BC$ and above $AB$.
These requirements~(i) and (ii) imply continuity of $f(x)$ on $x = x_1$.
Since $x_1$ can be any element in $\mathcal{X}$ (except $\partial\mathcal{X}$),
we see that $f(x)$ is continuous (except on $\partial\mathcal{X}$).}
function defined on a \emph{convex set}~$\mathcal{X}$ such that,
if $x_0, x_1 \in \mathcal{X}$,
then $\lambda x_0 + (1 - \lambda) x_1 \in \mathcal{X}$
for any $0 < \lambda < 1$.
The function~$f(x)$ is said to be \emph{convex} if
\begin{equation}
f(\lambda x_0 + (1 - \lambda) x_1)
\leqslant
\lambda f(x_0) + (1 - \lambda) f(x_1)
\label{eq:inequality_for_convexity}
\end{equation}
for any $x_0, x_1 \in \mathcal{X}$ and $0 < \lambda < 1$.
If the equality in \eqref{eq:inequality_for_convexity} holds only if $x_0 = x_1$,
then $f(x)$ is said to be \emph{strictly convex}.
If $f(x)$ is twice differentiable, the following properties hold.
\begin{itemize}
\item
The inequality condition~\eqref{eq:inequality_for_convexity} for convexity is equivalent to
the requirement that the second derivative~$f''(x)$ be everywhere nonnegative.
\item
If $f''(x)$ is (strictly) positive everywhere, then $f(x)$ is strictly convex.
Note however that the converse of this does not hold (consider, e.g., $f(x) = x^4$,
which is stritly convex but $f''(0) = 0$).
\end{itemize}
...
so that one could possibly assume an inappropriate model, e.g.,
a Bayesian regression model of Section~3.3 where the prior distribution~(3.52) is too broad,
which effectively leads to insufficient regularization.
Moreover, one should also be aware of a subtlety here that
(a) the \emph{generalization error}, which can be measured by cross-validation, and
(b) the \emph{model evidence} (or the \emph{marginal likelihood})
