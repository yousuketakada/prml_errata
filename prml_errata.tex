...
\end{align}
where we have used the \emph{multivariate gamma function}~$\Gamma_D(\cdot)$ given by
\eqref{eq:multivariate_gamma_function_in_terms_of_univariate_gamma_function},
by which the normalization constant~$B\left(\mathbf{W}, \nu\right)$ can be written as
...
It is a generalization of the univariate gamma function~$\Gamma(\cdot)$ given by (1.141).
...
\end{equation}
where $\mathbf{W} \in \mathcal{S}_{+}^{D}$ and $\nu > D - 1$.
...
\end{equation}
where $\psi(\cdot)$ is the univariate digamma function given by \eqref{eq:digamma_function},
then the log determinant expectation~(B.81) of the Wishart distribution can be written as
\begin{equation}
