...
$\left|f(x)/g(x)\right|$ should read $\left|g(x)/f(x)\right|$ (with the function names swapped).
See also \citet{NIST:DLMF} and note that
the function~$f(x)$ in the \emph{big O notation}~$g(x) = O\left(f(x)\right)$
is not necessarily ``tightest'' so that, e.g.,
we can write $ax^2 + bx + c = O(x^2)$
where the coefficient~$a$ for $x^2$ can be either nonzero ($a \neq 0$) or zero ($a = 0$).
...
which occurs where $q(\mathbf{x}) = 0$.
To avoid these difficulties, we should take a different approach~\citep{MacKay:Information}
in which we identify $f(\xi) = \xi\ln\xi$ and $\xi(\mathbf{x}) = p(\mathbf{x})/q(\mathbf{x})$;
and then make use of \eqref{eq:Jensen_inequality_continuous} with respect to $q(\mathbf{x})$.

To this end, let us first examine the behavior of the integrand of
...
such that $p(\mathbf{x}) = 0$ regardless of the values of $q(\mathbf{x})$
(recall that we have defined $0 \ln 0 = 0$ in the context of the \emph{entropy} on Page~49).
...
only over $\Omega = \left\{ \mathbf{x} \middle| q(\mathbf{x}) > 0 \right\}$.
...
\mathrm{d}\mathbf{x}
= \int_{\Omega} q(\mathbf{x})
f\left(\frac{p(\mathbf{x})}{q(\mathbf{x})}\right)
\mathrm{d}\mathbf{x} \\
&\geqslant f\left(\int_{\Omega} p(\mathbf{x}) \, \mathrm{d}\mathbf{x} \right)
= f(1) = 0
\end{align}
...
so is $f(\xi) = \xi\ln\xi$ for $\xi \geqslant 0$.
