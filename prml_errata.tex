...
is zero on average in any direction~$\bm{\eta}$
by noting that the expectation of the score function~\eqref{eq:score_function}
vanishes so that
\begin{equation}
...
It is worth noting here that we have a subtlety in
the identity~\eqref{eq:expected_score_function_vanishes}, that is,
when we introduce the score function~\eqref{eq:score_function},
...
the multinoulli distribution~$\operatorname{Mult}\left(\mathbf{x}\middle|\bm{\mu}\right)$
given by \eqref{eq:multinoulli_distribution}.
Since we cannot change a single parameter~$\mu_k$ independently of the others~$\mu_j$
where $j \neq k$ due to the constraint~$\sum_k \mu_k = 1$,
it is not valid to substitute
$\nabla_{\mu_k} \ln \operatorname{Mult}\left(\mathbf{x}\middle|\bm{\mu}\right)$
into \eqref{eq:expected_score_function_vanishes}.
Instead, we should consider the derivatives with respect to, say,
unnormalized probabilities~$\widetilde{\mu}_k \geqslant 0$,
in which case the score function is given by
\begin{equation}
...
we indeed obtain a valid result that $\mathbb{E}[x_k] = \mu_k$ where $\mu_k$ is given by 
\eqref{eq:normalized_mu_from_unnormalized_mu}.
