...
\end{equation}
where we have defined the \emph{support}~$\operatorname{supp}(\phi)$ of
a real-valued function~$\phi : \mathcal{X} \to \mathbb{R}$ by
\begin{equation}
\operatorname{supp}(\phi) \equiv \{ \mathbf{x} \in \mathcal{X} \mid \phi(\mathbf{x}) \neq 0 \}
\end{equation}
so that $\operatorname{supp}(p) = \{ \mathbf{x} \in \mathcal{X} \mid p(\mathbf{x}) > 0 \}$
if $p : \mathcal{X} \to [0, \infty)$ is a probability density function.\footnote{%
...
in which we optimize $p$ or $q$ given $q$ or $p$, respectively,
so as to minimize $\operatorname{KL}\left( p \middle\| q \right)$,
...
(see Section~10.1.2).}
Notice here a subtlety regarding the set inclusion notation that
we write $A \subset B$ or $B \supset A$ when $a \in B$ for all $a \in A$
so that $A$ may equal $B$.\footnote{%
In fact, a set~$A$ equals another set~$B$ or $A = B$
if and only if $A \subset B$ or $A \supset B$.}

Assuming \eqref{eq:supp_p_contained_in_supp_q},
...
\begin{equation}
\varphi(a) \equiv \ln a - \psi(a) , \quad a > 0 .
