...
consult a discussion regarding the eigenvalue decomposition found in Appendix~C of PRML;
...
(1)~for any pair of the elements~$\mathbf{U}, \mathbf{U}' \in \mathcal{U}_{+}^{D}$,
the product is again in the group~$\mathcal{U}_{+}^{D}$
so that $\mathbf{U}\mathbf{U}' \in \mathcal{U}_{+}^{D}$; and
(2)~for any $\mathbf{U} \in \mathcal{U}_{+}^{D}$,
