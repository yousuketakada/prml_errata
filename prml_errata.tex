...
as the distribution over a square matrix (called the \emph{scatter matrix}) of the form
\begin{equation}
...
and $\mathbf{G} = \left(G_{ij}\right)$ are written element-wise, then we have
\begin{equation}
\begin{pmatrix}
U_{11} & U_{12} & \cdots & U_{1D} \\
0      & U_{22} & \cdots & U_{2D} \\
...
It is worth noting here that
the combined change of variables~\eqref{eq:changes_of_variables_for_wishart_normalization}
from $\bm{\Lambda} \in \mathcal{S}_{+}^{D}$ to $\mathbf{R} \in \mathcal{U}_{+}^{D}$
effectively makes the resulting random variables~$R_{ij}$
to be independently distributed so that
...
then we have,
through the change of variables~\eqref{eq:changes_of_variables_for_wishart_normalization},
\begin{equation}
...
\right]
\left[
\prod_{i < j} \mathcal{N}\left( R_{ij} \middle| 0, \frac{1}{2} \right)
\right]
\end{equation}
where we have further changed variables for the diagonal elements through
$\tau_{i} = R_{ii}^{2}$.
...
directly by identifying the multivariate gamma function to show the normalization of the Wishart.
