...
Note however that, since the conditional~$p(y|x)$ can be considered
...
and it is safe to say that the \emph{product rule}~(1.32) holds in general.
...
then the Kullback-Leibler divergence from $p(\mathbf{x})$ to $q(\mathbf{x})$
