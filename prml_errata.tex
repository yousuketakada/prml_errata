...
an equivalent multivariate Student's t-distribution,
we now have a natural question:
Which marginal form is better than the other?
To answer this, I would argue that,
if the marginalized models obtained by marginalizing out the latent variables are identical,
a model with fewer latent variables is always better than a model with more latent variables
...
when we optimize the model parameters by means of the EM algorithm (see Chapter~9) or
variational methods (Chapter~10).
Therefore, we conclude that (2.161) is better than
\eqref{eq:multivariate_t_marginalized_over_precision}.
...
is again isotropic. The same marginal distribution can also be obtained by marginalizing over
a univariate Wishart (gamma) prior so that
\begin{equation}
...
Then, we see that the ``covariance'' parameter~$\bm{\Sigma}$ of the corresponding multivariate
Wishart prior~$\mathcal{W}\left(\bm{\Lambda}\middle|\mathbf{W}, \nu\right)$ is however \emph{not}
equal to
$\widetilde{\sigma}^{2} \mathbf{I}$ but given by scaling it by a factor of
$\widetilde{\nu}/(\widetilde{\nu} - 1 + D)$ so that
\begin{equation}
\bm{\Sigma} = \left(\nu\mathbf{W}\right)^{-1} =
