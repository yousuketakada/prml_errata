...
additional errata for the English edition,
...
\erratum{Page~432}
The text after (9.13):
It should be noted for clarity that, as the prior~$p(\mathbf{z})$ given by (9.10) is
...
\end{equation}
where we have written $\gamma_k \equiv \gamma(z_k)$ and $\gamma(z_k)$ is given by (9.13).
The \emph{responsibilities} $\gamma_k$ can be directly found by inspecting the functional form of
...
\end{equation}
and noting that the categorical distribution can also be expressed in terms of
unnormalized probabilities as shown in \eqref{eq:categorical_distribution_w_unnormalized_mu}.
