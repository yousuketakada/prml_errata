...
the expected loss evaluated for the predicted target values (i.e., point estimates)
with some arbitrary loss function given (e.g., the squared error for regression).
Motivated from a Bayesian point of view, we can define it differently
so as to be a better criterion for assessing a model's predictive ability, i.e.,
as the expected negative log predictive distribution (see below for a precise definition),
which, to avoid ambiguity, we hereafter call the \emph{generalization loss}.
