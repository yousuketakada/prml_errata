...
\erratum{Page~33}
Paragraph~3:
The two information criteria for model selection mentioned here, namely,
...
(ii)~to find the ``true'' model from which the data set has been generated
(or to better explain the data set in terms of the \emph{marginal likelihood}; see Section~3.4),
respectively.\footnote{%
Since we cannot tell which goal (generalization or to find the ``true'' model)
is more ``Bayesian'' than the other,
the term ``Bayesian information criteria'' is a misnomer.}
However, the difference is not well-recognized in PRML;
