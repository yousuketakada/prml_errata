...
$p(\mathbf{x})$ must be \emph{absolutely continuous} with respect to $q(\mathbf{x})$,
meaning that $p(\mathbf{x}) = 0$ wherever $q(\mathbf{x}) = 0$;
this property is known as \emph{zero-forcing} in the context of variational inference
(see Section~10.1.2).\footnote{%
The \emph{zero-forcing} property
(i.e., the requirement of \emph{absolute continuity} of $p$ with respect to $q$)
...
Assuming absolute continuity of $p(\mathbf{x})$ with respect to $q(\mathbf{x})$,
...
which, together with the assumed absolute continuity, yields the equality condition that
...
\end{align}
where $0 < \lambda < 1$.
Here, we again assume absolute continuity for each pair of the distributions~$(p_i, q_i)$,
i.e., $p_i(\mathbf{x}) = 0$
