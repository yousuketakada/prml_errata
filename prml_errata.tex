...
Suffixes to the $q$ distributions are dropped;
we should write, e.g., $q_i(z_i)$ instead of $q(z_i)$.
Note that \citet{Svensen:PRML_errata} corrects (10.12) only.
Or, we should explicitly define that the variational distribution over $\mathbf{z}_i$ is denoted by
$q(\mathbf{z}_i)$ just as the probability~$p(\cdot)$ is determined by its argument.
