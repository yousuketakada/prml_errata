...
$\mathcal{T}_{+}^{D}$ forms a \emph{group} with respect to multiplication, i.e.,
for any pair of $\mathbf{L}, \mathbf{M} \in \mathcal{T}_{+}^{D}$,
their product is again in the group~$\mathcal{T}_{+}^{D}$ so that $\mathbf{L}\mathbf{M} \in \mathcal{T}_{+}^{D}$; and,
...
where
\begin{equation}
\bm{\lambda} = \widehat{\mathbf{M}}^{\operatorname{T}}\bm{\beta} + \alpha\bm{\eta} .
\end{equation}

Similarly, the second part can be shown by induction.
...
where
\begin{equation}
\bm{\xi} = -\frac{\widehat{\mathbf{L}}^{-\operatorname{T}}\bm{\beta}}{\alpha} .
\end{equation}
