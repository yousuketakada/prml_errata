...
one can see that the equality in \eqref{eq:Jensen_inequality_expectation_with_z}
...
(one can easily see that the t-distributions are not correctly normalized).
Figure~\ref{fig:Student_t_densities} gives the correct plot.

\begin{figure}
\centering
\includegraphics{student_t_plots.eps}~%
\includegraphics{student_t_plots_log.eps}
\caption{Plot of Student's t densities (left) and log densities (right)
...
More specifically, if we denote the right hand side of (5.84) by $\mathbf{H}_{\text{GN}}$,
the Levenberg-Marquardt approximation to the Hessian is given by
\begin{equation}
\mathbf{H}_{\text{LM}} = \mathbf{H}_{\text{GN}} + \lambda \mathbf{D}
\end{equation}
where $\lambda \geqslant 0$ is an adjustable damping factor and $\mathbf{D}$ is
some positive (semi)definite matrix.
The matrix~$\mathbf{D}$ typically takes the form $\mathbf{D} = \mathbf{I}$ or
$\mathbf{D} = \operatorname{diag}(\mathbf{H}_{\text{GN}})$
where we have defined that, for a square matrix~$\mathbf{A}$, $\operatorname{diag}(\mathbf{A})$ is
...
Without introducing an intermediate quantity like $\tilde{p}(\mathbf{X}, \mathbf{Z}_j)$,
...
in terms of unnormalized probabilities~$\tilde{\mu}_k \geqslant 0$, i.e.,
\begin{equation}
...
so that
\begin{equation}
\ln\operatorname{Mult}\left(\mathbf{x}\middle|\tilde{\bm{\mu}}\right) =
  \sum_{k=1}^{K} {x_k}\ln{\tilde{\mu}_k} + \text{const}
\end{equation}
where the normalization constant is given by
${C(\tilde{\bm{\mu}})}^{-1} = \sum_{k} \tilde{\mu}_k$
so that the normalized probabilities can be found as
\begin{equation}
\mu_k = p(x_k = 1) = \frac{\tilde{\mu}_k}{\sum_{j} \tilde{\mu}_j} .
\end{equation}
...
\end{equation}
One can show that,
if we use an isotropic scale matrix~$\mathbf{W} = W\mathbf{I}$, which is common in practice,
the resulting multivariate Student's t-distribution is also isotropic and,
up to some reparameterization,
the same marginal distribution can also be obtained by marginalizing over
a gamma (one-dimensional Wishart) distribution as
\begin{equation}
\int \mathcal{N}\left(\mathbf{x}\middle|\bm{\mu}, \lambda^{-1}\mathbf{I}\right)
\mathcal{W}\left(\lambda\middle|W, \nu\right) \mathrm{d}\lambda
= \operatorname{St}\left(\mathbf{x} \middle| \bm{\mu}, \nu W \mathbf{I}, \nu \right)
\end{equation}
where $W > 0$ and $\nu > 0$.
