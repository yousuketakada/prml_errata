...
(1)~the product of any pair of $\mathbf{T}, \mathbf{T}' \in \mathcal{T}_{+}^{D}$
is again in the group~$\mathcal{T}_{+}^{D}$
so that $\mathbf{T}\mathbf{T}' \in \mathcal{T}_{+}^{D}$; and
(2)~for any $\mathbf{T} \in \mathcal{T}_{+}^{D}$,
there exists an inverse~$\mathbf{T}^{-1} \in \mathcal{T}_{+}^{D}$ such that
$\mathbf{T}^{-1} \mathbf{T} = \mathbf{T} \mathbf{T}^{-1} = \mathbf{I}$
where $\mathbf{I} \in \mathcal{T}_{+}^{D}$ is the identity matrix.\footnote{%
The inverse~$\mathbf{T}^{-1}$ is unique in general.
To see this, suppose that $\mathbf{X}$ is another inverse of $\mathbf{T}$.
Then, we have by associativity that
$\mathbf{X} = \left( \mathbf{T}^{-1} \mathbf{T} \right) \mathbf{X} =
\mathbf{T}^{-1} \left( \mathbf{T} \mathbf{X} \right) = \mathbf{T}^{-1}$.}
...
For a general $D > 1$, let us write $\mathbf{T}, \mathbf{T}' \in \mathcal{T}_{+}^{D}$
as partitioned matrices so that
\begin{equation}
\mathbf{T} =
\begin{pmatrix}
\widehat{\mathbf{T}} & \bm{\beta} \\
...
\end{equation}
where $\widehat{\mathbf{T}}, \widehat{\mathbf{T}}' \in \mathcal{T}_{+}^{D - 1}$ and
$\alpha, \alpha' > 0$.
Assuming that the product of $\widehat{\mathbf{T}}$ and $\widehat{\mathbf{T}}'$ is again
in the group~$\mathcal{T}_{+}^{D - 1}$ so that
$\widehat{\mathbf{T}}\widehat{\mathbf{T}}' \in \mathcal{T}_{+}^{D - 1}$,
we have
\begin{equation}
\mathbf{T}\mathbf{T}' =
\begin{pmatrix}
...
=
\begin{pmatrix}
\widehat{\mathbf{T}}\widehat{\mathbf{T}}' & \bm{\eta} \\
\mathbf{0}^{\operatorname{T}} & \alpha\alpha'
\end{pmatrix} \in \mathcal{T}_{+}^{D}
\end{equation}
where
\begin{equation}
\bm{\eta} = \widehat{\mathbf{T}}\bm{\beta}' + \bm{\beta}\alpha' .
\end{equation}

The second part~(2) can be shown similarly by induction.
Again, it is trivial to show the case where $D = 1$; and,
for a general $D > 1$, we make use of partitioned matrices.
Assuming that the inverse~$\widehat{\mathbf{T}}^{-1}$ of
$\widehat{\mathbf{T}} \in \mathcal{T}_{+}^{D - 1}$ is again in the group~$\mathcal{T}_{+}^{D - 1}$
so that $\widehat{\mathbf{T}}^{-1} \in \mathcal{T}_{+}^{D - 1}$,
we easily find $\mathbf{T}^{-1}$ in the form
\begin{equation}
\mathbf{T}^{-1} =
\begin{pmatrix}
\widehat{\mathbf{T}} & \bm{\beta} \\
...
\end{pmatrix}^{-1} =
\begin{pmatrix}
\widehat{\mathbf{T}}^{-1} & \bm{\xi} \\
\mathbf{0}^{\operatorname{T}} & \alpha^{-1}
\end{pmatrix} \in \mathcal{T}_{+}^{D}
\end{equation}
where
\begin{equation}
\bm{\xi} = -\widehat{\mathbf{T}}^{-1} \bm{\beta} \alpha^{-1} .
\end{equation}

\parhead{Cholesky decomposition}
For any symmetric positive-definite matrix~$\mathbf{A} \in \mathcal{S}_{+}^{D}$,
there exists a unique upper triangular matrix~$\mathbf{T} \in \mathcal{T}_{+}^{D}$
such that
\begin{equation}
\mathbf{A} = \mathbf{T}^{\operatorname{T}} \mathbf{T}
\label{eq:cholesky_decomposition}
\end{equation}
or, if the matrices~$\mathbf{A} = \left(A_{ij}\right)$ and $\mathbf{T} = \left(T_{ij}\right)$ are
...
\begin{pmatrix}
T_{11} & 0      & \cdots & 0      \\
T_{12} & T_{22} & \ddots & \vdots \\
\vdots & \vdots & \ddots & 0      \\
T_{1D} & T_{2D} & \cdots & T_{DD}
\end{pmatrix}
\begin{pmatrix}
T_{11} & T_{12} & \cdots & T_{1D} \\
0      & T_{22} & \cdots & T_{2D} \\
...
The upper triangular matrix~$\mathbf{T}$ in
the right hand side of \eqref{eq:cholesky_decomposition}
is called the \emph{Cholesky factor}\footnote{%
The Cholesky decomposition~\eqref{eq:cholesky_decomposition}
can also be (and is more often) written in the form
\begin{equation}
\mathbf{A} = \mathbf{L} \mathbf{L}^{\operatorname{T}}
\end{equation}
where $\mathbf{L} = \mathbf{T}^{\operatorname{T}}$,
in which case the lower triangular matrix~$\mathbf{L}$ is called the Cholesky factor.}.

Conversely, for any upper triangular matrix~$\mathbf{T} \in \mathcal{T}_{+}^{D}$,
...
indeed agrees with that of $\mathbf{T} \in \mathcal{T}_{+}^{D}$.
More specifically, either of $\mathbf{A}$ and $\mathbf{T}$ has $D(D + 1)/2$ independent parameters
because $\mathbf{A}$ is a symmetric matrix and $\mathbf{T}$ is an (upper) triangular matrix.
...
$T = \sqrt{A} > 0$ satisfies \eqref{eq:cholesky_decomposition}.
...
let us write $\mathbf{A} \in \mathcal{S}_{+}^{D}$ and $\mathbf{T} \in \mathcal{T}_{+}^{D}$
as partitioned matrices so that
\begin{equation}
\mathbf{A} =
\begin{pmatrix}
...
$\widehat{\mathbf{T}} \in \mathcal{T}_{+}^{D - 1}$.\footnote{%
...
\begin{equation}
\mathbf{T}^{\operatorname{T}}\mathbf{T} =
\begin{pmatrix}
\widehat{\mathbf{T}}^{\operatorname{T}} & \mathbf{0} \\
...
=
\begin{pmatrix}
\widehat{\mathbf{T}}^{\operatorname{T}}\widehat{\mathbf{T}} &
\widehat{\mathbf{T}}^{\operatorname{T}}\bm{\beta} \\
\bm{\beta}^{\operatorname{T}}\widehat{\mathbf{T}} &
\|\bm{\beta}\|^{2} + \alpha^{2}
...
\begin{align}
\bm{\beta} &= \widehat{\mathbf{T}}^{-\operatorname{T}} \mathbf{b}
...
$\mathbf{T} \in \mathcal{T}_{+}^{D}$ from $\mathbf{A} \in \mathcal{S}_{+}^{D}$.
