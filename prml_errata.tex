...
the trace~$\operatorname{Tr}(\cdot)$, i.e.,
\begin{equation}
...
\end{equation}
It follows from \eqref{eq:partial_derivative_of_log_abs_det} that,
if we adopt the notation~\eqref{eq:abs_of_matrix_determinant} for $\left|\mathbf{A}\right|$,
the identity~\eqref{eq:gradient_of_log_abs_det_A_wrt_A} holds for
