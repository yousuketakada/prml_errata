...
With the original, uncorrected definition of
the \emph{big O notation}~$g(x) = O\left(f(x)\right)$ in PRML,
we would have, say, $x^2 = O(x)$ because $\left|x/x^2\right|$ is bounded as $x \to \infty$,
but \emph{not} $x = O(x^2)$ because $\left|x^2/x\right|$ is unbounded;
we should have exactly the opposite.
See also \citet{NIST:DLMF} for other asymptotic and order notations.
...
Assuming the zero-forcing condition, i.e.,
...
find an alternative form of the log expectation~$\mathbb{E}\left[ \ln\lambda \right]$
in terms of the logarithm of the mean~$\ln\mathbb{E}[\lambda]$.

For any correctly normalized probability distribution~$p\left(\mathbf{x}\middle|\bm{\theta}\right)$
over random variables~$\mathbf{x}$
parameterized by $\bm{\theta}$ and differentiable with respect to $\bm{\theta}$,
we have that the expectation of its \emph{Fisher score}~(6.32),
i.e., the derivative of the log probability with respect to $\bm{\theta}$,
vanishes so that
\begin{equation}
...
\end{equation}
We can show the above identity~\eqref{eq:expected_fisher_score}
...
into \eqref{eq:expected_fisher_score}.
...
can be evaluated exactly in this case so that
\begin{equation}
...
the maximum likelihood solution for the gamma distribution as we shall see shortly.
Note also that we make use of \eqref{eq:log_minus_digamma_function}
in the EM algorithm for inferring the (multivariate) Student's t-distribution~(2.162),
which is concerned in Exercise~12.24.
...
\erratum{Page~104}
Paragraph~1, Line~4:
In practical applications, the importance of \emph{robustness to outliers}
cannot be overemphasized.
Particularly in the context of robust regression,
there exist historically a number of heuristic approaches such as
\emph{M-estimators}~\citep{Press:NR,Szeliski:ComputerVision},
in which the least squares method is modified so as to use a more ``robust'' cost function.
One can show that the robust regression problem can be solved by
approximating the log likelihood function in terms of quadratic bounds,
which results in the same algorithm as the EM (see Chapter~9),
which is also known as \emph{iteratively reweighted least squares} or IRLS.
Note that this quadratic approximation is closely related to the local variational methods
...
Specifically, the digamma function~\citep{AS:Handbook,NIST:DLMF} is given by
\begin{equation}
