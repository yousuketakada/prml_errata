...
\end{equation}
Note that the above identity~\eqref{eq:determinant_of_upper_triangular_matrix}
can also be shown directly from the definition~(C.10) of the determinant.\footnote{%
...
\eqref{eq:integral_identity_for_normalization_constant_of_wishart}
by making the changes of variables~\eqref{eq:changes_of_variables_for_wishart_normalization},
we have
\begin{align}
&\int_{\mathcal{S}_{+}^{D}}
\left| \bm{\Lambda} \right|^{(v - D - 1)/2}
