...
Note that,
although matrix determinant~$\operatorname{det}(\cdot)$ and trace~$\operatorname{Tr}(\cdot)$
only apply to square matrices,
matrices~$\mathbf{A}$, $\mathbf{B}$, and $\mathbf{C}$ in (C.8) and (C.9) themselves are
...
we write the \emph{gradient} of $\mathbf{y}(\mathbf{x})$ with respect to $\mathbf{x}$ as
\begin{equation}
...
a column vector~$\mathbf{x}$ is again a column vector of the same dimensionality,
...
together with \eqref{eq:gradient_of_matrix_vector_product}
when, e.g.,
...
using \eqref{eq:gradient_of_quadratic_form_symmetric} and
...
Evaluating the gradient and the Hessian of the error function~$J$
...
we have
\begin{align}
\nabla_{\bm{\theta}} J &= \sum_{n=1}^{N} \varepsilon_n \nabla_{\bm{\theta}}\, \varepsilon_n
= \left( \nabla_{\bm{\theta}}\, \bm{\varepsilon} \right) \bm{\varepsilon} \\
\nabla_{\bm{\theta}}\nabla_{\bm{\theta}} J &= \sum_{n=1}^{N}
\nabla_{\bm{\theta}}\, \varepsilon_n
\left(\nabla_{\bm{\theta}}\, \varepsilon_n\right)^{\operatorname{T}}
+ \sum_{n=1}^{N} \varepsilon_n \nabla_{\bm{\theta}}\nabla_{\bm{\theta}}\, \varepsilon_n \\
&= \nabla_{\bm{\theta}}\, \bm{\varepsilon}
\left( \nabla_{\bm{\theta}}\, \bm{\varepsilon} \right)^{\operatorname{T}}
+ \left[ \nabla_{\bm{\theta}}
\operatorname{vec}\left(\nabla_{\bm{\theta}}\, \bm{\varepsilon}\right) \right]
...
\end{pmatrix}
\end{equation}
where $\mathbf{V} = \left(\mathbf{v}_1, \dots, \mathbf{v}_D \right)$;
...
of the Hessian~$\nabla_{\bm{\theta}}\nabla_{\bm{\theta}} J$,
which, however, does not necessarily lead to efficient implementation
(neither does that of the gradient~$\nabla_{\bm{\theta}} J$),
can be directly obtained from a general product rule of the form
\begin{align}
\nabla_{\mathbf{x}} \left\{ \mathbf{R} \bm{\phi} \right\}
&= \nabla_{\mathbf{x}} \bm{\phi} \, \mathbf{R}^{\operatorname{T}}
+ \nabla_{\mathbf{x}} \operatorname{vec}\left( \mathbf{R} \right)
\left( \bm{\phi} \otimes \mathbf{I}_M \right)
\label{eq:gradient_of_matrix_vector_product_general}
\\
&= \sum_{n=1}^{N} \nabla_{\mathbf{x}} \phi_n \, \mathbf{r}_n^{\operatorname{T}}
+ \sum_{n=1}^{N} \phi_n \nabla_{\mathbf{x}} \mathbf{r}_n
\end{align}
where $\mathbf{R} = \left(\mathbf{r}_1, \dots, \mathbf{r}_N\right)$,
