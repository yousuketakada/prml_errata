...
the covariance~$\bm{\Sigma}$ and the precision~$\bm{\Lambda}$ of
...
the concept of positive/negative (semi)definiteness is restricted to symmetric matrices, i.e.,
any positive/negative (semi)definite matrix is symmetric.  }.
...
When we construct a probabilistic model with Wishart distributions,
we must choose reasonable initial values
for such (hyper)parameters, which requires their intuitive interpretation.
