...
meaning that matrix operations in \eqref{eq:gradient_of_Tr_AB_wrt_A} are meaningful.
...
\end{equation}
where the Jacobian matrix~$\partial\mathbf{A}/\partial A_{ij}$ is,
by definition~\eqref{eq:Jacobian_of_matrix_wrt_scalar},
of the same dimensionality as $\mathbf{A}$ and
such that only the $(ij)$-th element is one and all the other elements are zero so that
\begin{equation}
...
The identity~\eqref{eq:gradient_of_Tr_Ainverse_B} follows similarly from (C.21).
...
Let $\varphi(\mathbf{A})$ be a scalar function of a square matrix~$\mathbf{A}$
where $\mathbf{A} = \left(A_{ij}\right)$ is not a symmetric matrix so that
$A_{ij} \not\equiv A_{ji}$.
As usual, we write the gradient of $\varphi(\mathbf{A})$ with respect to $\mathbf{A}$ as
\begin{equation}
\nabla_{\mathbf{A}} \varphi(\mathbf{A})
= \left( \frac{\partial}{\partial A_{ij}} \varphi(\mathbf{A}) \right) .
\end{equation}
Suppose that we want to evaluate the gradient of $\varphi(\mathbf{S})$ where
$\mathbf{S} = \left(S_{ij}\right)$ is a symmetric matrix so that $S_{ij} \equiv S_{ji}$.
The derivative of $\varphi(\mathbf{S})$ with respect to
an off-diagonal element~$S_{ij}$ where $i \neq j$
consists of two derivatives through $A_{ij}$ and $A_{ji}$ so that
\begin{equation}
\frac{\partial}{\partial S_{ij}} \varphi(\mathbf{S})
= \frac{\partial}{\partial A_{ij}} \varphi(\mathbf{S})
+ \frac{\partial}{\partial A_{ji}} \varphi(\mathbf{S})
...
\frac{\partial}{\partial A_{ij}} \varphi(\mathbf{S})
\equiv \left.
\frac{\partial}{\partial A_{ij}} \varphi(\mathbf{A}) \right|_{\mathbf{A} = \mathbf{S}} .
\end{equation}
The derivative of $\varphi(\mathbf{S})$ with respect to an diagonal element~$S_{ii}$ is given by
\begin{equation}
\frac{\partial}{\partial S_{ii}} \varphi(\mathbf{S})
= \frac{\partial}{\partial A_{ii}} \varphi(\mathbf{S}) .
\label{eq:derivative_of_scalar_wrt_S_ii}
\end{equation}
Thus, we can write
\begin{equation}
\nabla_{\mathbf{S}} \varphi(\mathbf{S})
= \nabla_{\mathbf{A}} \varphi(\mathbf{S})
+ \nabla_{\mathbf{A}} \varphi(\mathbf{S})^{\operatorname{T}}
- \operatorname{diag}\left( \nabla_{\mathbf{A}} \varphi(\mathbf{S}) \right)
...
if $\nabla_{\mathbf{A}} \varphi(\mathbf{S})$ is symmetric (which holds commonly in practice),
we have
\begin{equation}
\nabla_{\mathbf{S}} \varphi(\mathbf{S}) = \mathbf{O}
\quad \Longleftrightarrow \quad
\nabla_{\mathbf{A}} \varphi(\mathbf{S}) = \mathbf{O}
\end{equation}
which implies that we can solve $\nabla_{\mathbf{S}} \varphi(\mathbf{S}) = \mathbf{O}$
without the symmetry constraint on $\mathbf{S}$, i.e.,
by solving $\nabla_{\mathbf{A}} \varphi(\mathbf{S}) = \mathbf{O}$
and then obtain a solution~$\mathbf{S}$ that is symmetric.
...
if $\mathbb{E}\left[\nabla_{\mathbf{A}} \varphi(\mathbf{S})\right]$ is symmetric,
we have
\begin{equation}
\mathbb{E}\left[\nabla_{\mathbf{S}} \varphi(\mathbf{S})\right] = \mathbf{O}
\quad \Longleftrightarrow \quad
\mathbb{E}\left[\nabla_{\mathbf{A}} \varphi(\mathbf{S})\right] = \mathbf{O} .
\label{eq:equivalence_of_expected_gradient_wrt_symmetric_matrix_equal_to_zero}
\end{equation}
Strictly speaking, since the score function~\eqref{eq:score_function}
occurring in \eqref{eq:expected_score_function_vanishes}
can be defined only for independent parameters,
if the parameters of interest are, say, a symmetric matrix,
we must impose the symmetry constraint on the parameters.
The equivalence~\eqref{eq:equivalence_of_expected_gradient_wrt_symmetric_matrix_equal_to_zero}
allows us to safely ignore the symmetry constraint and use
$\mathbb{E}\left[\nabla_{\mathbf{A}} \varphi(\mathbf{S})\right] = \mathbf{O}$ instead.
...
most software implementations of symmetric eigenvalue decomposition only guarantee that
...
dimensionality~$M \times M$ and $N \times N$, respectively;
