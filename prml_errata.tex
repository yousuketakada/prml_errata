...
The logarithmic expectation~$\mathbb{E}\left[ \ln\lambda \right]$ can be most easily calculated
...
\erratum{Page~690}
Equation~(B.54):
The discrete distribution of the form~(B.54), or (2.26), has been (perhaps confusingly) given
several different names such as ``the discrete distribution,''
the \emph{categorical} distribution, and
the \emph{multinoulli} distribution~\citep{Murphy:MachineLearning}.
Of these terms, I would prefer the term \emph{multinoulli}
because it easily suggests that it is a generalization of the Bernoulli distribution~(B.1)
...
Moreover, the marginal of the form~(2.161) is even better in terms of the modeling flexibility
because it allows us to lean the mean~$\bm{\mu}$ and the precision~$\bm{\Lambda}$ parameters
