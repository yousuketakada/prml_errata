...
The conditional~$p(y|x)$ can be regarded as
a random variable dependent on the random variable~$x$ on which it is conditioned.
As such, we see that
there is no particular problem if $p(y|x)$ is undefined for $x$ such that $p(x) = 0$
and the \emph{product rule}~(1.32) holds in general.
