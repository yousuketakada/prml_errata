...
we are also interested in the log expectation~$\mathbb{E}\left[ \ln\lambda \right]$
...
concerned in Exercise~2.11 (the mean and the covariance are in Exercise~2.10).
Following a similar pattern, we can calculate the log expectation of
...
f(\mathbf{x})
+ \bm{\epsilon}^{\operatorname{T}} \mathbf{g}
+ \frac{1}{2} {\bm{\epsilon}}^{\operatorname{T}} \mathbf{H} \bm{\epsilon}
\end{equation}
