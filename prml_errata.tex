...
\erratum{Page~279}
Equation~(5.167):
The conditioning on $\mathcal{D}$ (or on other variables) for
an approximate distribution~$q(\cdot)$ does not make sense unless properly defined.
Hence, the conditioning for $q(\cdot)$ should be removed or
the variables on which conditioned should instead be specified as parameters
