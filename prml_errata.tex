...
in order to obtain a more general result
\begin{equation}
...
\end{equation}
where we have interpreted $\lambda_i \equiv p(\mathbf{z} = \mathbf{z}_i)$ and
$x_i \equiv \xi(\mathbf{z}_i)$.
...
\end{equation}
where we have assumed that $f(\cdot)$ is a convex function but $\xi(\cdot)$ can be any.
Note that, if $f(\cdot)$ is a strictly convex function,
the equality in \eqref{eq:Jensen_inequality_expectation} holds if and only if
$\xi(\cdot)$ is constant.
