...
\prod_{n=1}^{N} \mathcal{N}\left(\mathbf{x}_n\middle|\bm{\mu}, \bm{\Lambda}\right)
\end{equation}
...
\end{equation}
At this moment, we define the maximum likelihood estimates for the mean and the covariance
given the $N$ observations~$\mathbf{X} = \left\{ \mathbf{x}_1, \dots, \mathbf{x}_N \right\}$ as
\begin{equation}
\bm{\mu}_{\operatorname{ML}} = \frac{1}{N} \sum_{n=1}^{N} \mathbf{x}_n, \qquad
\bm{\Sigma}_{\operatorname{ML}} =
\frac{1}{N} \sum_{n=1}^{N}
(\mathbf{x}_n - \bm{\mu}_{\operatorname{ML}})
(\mathbf{x}_n - \bm{\mu}_{\operatorname{ML}})^{\operatorname{T}}
\end{equation}
respectively.
...
\begin{align}
\beta_N &= \beta_0 + N \\
\beta_N\bm{\mu}_N &= \beta_0\bm{\mu}_0 + N\bm{\mu}_{\operatorname{ML}} \\
\nu_N &= \nu_0 + N \\
\mathbf{W}_{N}^{-1} &= \mathbf{W}_{0}^{-1} + N \bm{\Sigma}_{\operatorname{ML}} +
\frac{\beta_0 N}{\beta_0 + N}
\left(\bm{\mu}_{\operatorname{ML}} - \bm{\mu}_0\right)
\left(\bm{\mu}_{\operatorname{ML}} - \bm{\mu}_0\right)^{\operatorname{T}} .
...
\mathcal{W}\left(\bm{\Lambda}\middle|\mathbf{W}_N, \nu_N\right) .
\end{equation}
This result suggests us that
we have parameters for the mean $\beta_0$ and $\bm{\mu}_0$ that are basically separate from
those for the precision (or, equivalently, the covariance) $\nu_0$ and $\mathbf{W}_0$.
We can interpret $\beta_0$ as the number of ``effective'' observations for the mean and
...
the one we have obtained in the previous erratum except that
we have in \eqref{eq:Gaussian-Wishart_W_0} a term that corresponds to the uncertainty in the mean.
...
the ``(Vector and) Matrix Derivatives'' section of Appendix~C.\footnote{At this point,
...
in which case it is no longer possible in general to take the orthogonal matrix~$\mathbf{U}$
