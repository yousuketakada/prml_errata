...
(1)~for any pair of $\mathbf{L}, \mathbf{M} \in \mathcal{T}_{+}^{D}$,
their product is again in the group~$\mathcal{T}_{+}^{D}$
...
Furthermore, we do not in general have \eqref{eq:definition_of_positive_definiteness}
...
\end{pmatrix} \not\in \mathcal{S}_{+}^{2}
\end{equation}
so that we have, e.g., $\mathbf{x}^{\operatorname{T}}\mathbf{P}\mathbf{x} = -1 < 0$
...
Similarly, the second part~(2) can also easily be shown by induction.
...
we can find the inverse~$\mathbf{L}^{-1}$ of $\mathbf{L} \in \mathcal{T}_{+}^{D}$ in the form
\begin{equation}
\mathbf{L}^{-1} =
\begin{pmatrix}
\widehat{\mathbf{L}} & \mathbf{0} \\
...
The lower triangular matrix~$\mathbf{L}$ is called the Cholesky factor.
...
Note also that the number of independent parameters in $\mathbf{A} \in \mathcal{S}_{+}^{D}$
...
\end{pmatrix} .
\end{equation}
Equating this with $\mathbf{A}$ gives
\begin{align}
...
\end{align}
We can show $a - \|\bm{\beta}\|^{2} > 0$ by substituting
\begin{equation}
\mathbf{x} =
\begin{pmatrix}
