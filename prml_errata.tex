...
In order to be specific, let us denote the right hand side of (5.84) by $\mathbf{H}_{\text{GN}}$.
The Gauss-Newton approximation~$\mathbf{H}_{\text{GN}}$ to the Hessian
can also be written as
\begin{equation}
\mathbf{H}_{\text{GN}} = \mathbf{J}^{\operatorname{T}}\mathbf{J}
\end{equation}
where $\mathbf{J} = \left( \nabla y_1, \dots, \nabla y_N \right)^{\operatorname{T}}$ is
the Jacobian matrix of the outputs~$y_1, \dots, y_N$ with respect to the parameters
(i.e., weights and biases).
The Levenberg-Marquardt method typically corrects the above approximate Hessian into the form
\begin{equation}
...
(which is adjusted through the iterations) and defined that,
for a square matrix~$\mathbf{A}$, $\operatorname{diag}(\mathbf{A})$ is
