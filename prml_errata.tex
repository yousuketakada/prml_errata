...
\end{equation}
which is necessary to show the \emph{push-through identity}~(C.6).
...
To this end, we first note that a partitioned matrix
\begin{equation}
\mathbf{M} =
\begin{pmatrix}
\mathbf{A} & \mathbf{B} \\
...
\end{equation}
respectively.
These identities~\eqref{eq:block_diagonalization_of_partitioned_matrix_with_Schur_complement_wrt_A}
