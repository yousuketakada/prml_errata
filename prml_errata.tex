...
(the third printing only).\footnote{%
...
Such errors are marked ``(the third printing only)'' in this report.}
...
because it is not a function of the product of $m_1, m_2, \dots, m_K$.
...
% Due to Mark-Jan Nederhof
Paragraph~1, Line~1:
``linear Gaussian'' should read ``linear-Gaussian'' for consistency with other part of PRML
(see, e.g., Section~8.1.4).
...
\end{equation}
Perhaps we should have used another notation
because we use $\widetilde{E}$ differently in Section 5.5.5,
though no big confusion will occur if we use $\widetilde{E}(\mathbf{w})$ here.
...
``$\delta$s'' should read ``$\delta$'s'' for consistency; see the line above (5.65).
...
By the way, such an unlabeled equation makes me upset;
it is simply difficult to refer to.
...
\erratum{Page~279}
Equation~(5.166):
The conditioning on $\mathcal{D}$ for an approximate distribution does not make sense and
is best removed (but we can still add $\mathcal{D}$ as a parameter) so that
the left hand side of (5.167) should read
$q(\mathbf{w}; \mathcal{D}, \alpha, \beta)$ or, simply,
$q(\mathbf{w})$.
...
Although the sign~$\simeq$ is always used for any type of approximate equality in PRML,
I make some distinction between $\simeq$ and $\approx$ in this report:
I use $\simeq$ for series expansions where approximation can be exact at some point;
whereas $\approx$ is used for general approximation.}
...
% See http://ibisforest.org/index.php?PRML%2Ferrata2
Paragraph~1, Line~2:
Remove ``a'' in ``with a Gaussian processes.''
...
the Wishart distribution~(B.78);
it is first introduced in Section~2.3.6 as
