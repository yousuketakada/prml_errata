...
can be expressed in terms of unnormalized probabilities~$\widetilde{\mu}_k$ as shown in
\eqref{eq:categorical_distribution_w_unnormalized_mu} where
the normalized probabilities~$\mu_k$ are given by \eqref{eq:normalized_mu_from_unnormalized_mu}.
...
\erratum{Page~467}
Text after (10.11), Equation~(10.12), and text after (10.15):
Suffixes to $q$ are dropped occasionally;
we should write $q_i(z_i)$ instead of $q(z_i)$ for example.
