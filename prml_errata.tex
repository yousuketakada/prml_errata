...
\Section{Preface}
This report communicates some more errata for \emph{Pattern Recognition and Machine Learning} or
...
I would like to thank those who have kindly informed me of
yet more errata and clarifications incorporated in this report.
...
if we regard the entropy~$\operatorname{H}[\cdot]$ as a functional,
we see that ``the entropy could equally well have been written as $\operatorname{H}[p]$.''
...
so that the entropy~$\operatorname{H}[x]$~(1.93) is well-defined.}
...
the \emph{conditional expectation}~(1.37) so that
...
the Wishart distribution~(B.78) because it has been introduced in Section~2.3.6 as
the conjugate prior~(2.155) for the precision matrix~$\bm{\Lambda}$ of
the multivariate Gaussian distribution~%
$\mathcal{N}\left(\mathbf{x}\middle|\bm{\mu}, \bm{\Lambda}^{-1}\right)$
(where the mean~$\bm{\mu}$ is assumed to be known)
but no proof has been given for the normalization constant~(2.156) or (B.79).
...
as the distribution over a matrix (called the \emph{scatter matrix}) of the form
\begin{equation}
...
In the following,
I would rather like to show the normalization~(B.79) as well as the expectations~(B.80) and (B.81)
...
similar to the one that we apply for calculating the multivariate gamma function.
The expectations~(B.80) and (B.81) are evaluated by making use of
...
$\mathcal{T}_{+}^{D} \subset \mathbb{R}^{D \times D}$ be
the space of \emph{lower triangular} matrices with positive diagonal elements.
Here, a lower triangular matrix~$\mathbf{L} = \left(L_{ij}\right)$
is a square matrix such that $L_{ij} \equiv 0$ where $i < j$ so that
\begin{equation}
\mathbf{L} =
\begin{pmatrix}
L_{11} & 0      & \cdots & 0      \\
...
\end{pmatrix}
\end{equation}
where $D$ is the dimensionality of $\mathbf{L}$.

If $\mathbf{L} \in \mathcal{T}_{+}^{D}$,
then $\mathbf{L} = \left(L_{ij}\right)$ is a lower triangular matrix of dimensionality~$D$
such that $L_{ii} > 0$ for all $i$.
...
The plus sign~($+$) in the subscripts of $\mathcal{S}_{+}^{D}$ and $\mathcal{T}_{+}^{D}$
indicates that any matrix in either of the sets is such that
all the eigenvalues are (strictly) positive.
To see this for $\mathbf{A} \in \mathcal{S}_{+}^{D}$, consult Appendix~C of PRML;
for $\mathbf{L} \in \mathcal{T}_{+}^{D}$, consider its \emph{characteristic equation}~(C.30),
from which it readily follows that the eigenvalues of $\mathbf{L} = \left(L_{ij}\right)$ are
equal to its diagonal elements~$L_{ii} > 0$.

\parhead{Triangular matrix groups}
It is worth noting here that
$\mathcal{T}_{+}^{D}$ forms a \emph{group} with respect to matrix multiplication, i.e.,
(1)~the product of any pair of $\mathbf{L}, \mathbf{M} \in \mathcal{T}_{+}^{D}$
is again in the group~$\mathcal{T}_{+}^{D}$
so that $\mathbf{L}\mathbf{M} \in \mathcal{T}_{+}^{D}$; and
(2)~for any $\mathbf{L} \in \mathcal{T}_{+}^{D}$,
there exists an inverse~$\mathbf{L}^{-1} \in \mathcal{T}_{+}^{D}$ such that
$\mathbf{L}^{-1} \mathbf{L} = \mathbf{L} \mathbf{L}^{-1} = \mathbf{I}$
where $\mathbf{I} \in \mathcal{T}_{+}^{D}$ is the identity matrix.\footnote{%
The inverse~$\mathbf{L}^{-1}$ is unique in general.
To see this, suppose that $\mathbf{M}$ is another inverse of $\mathbf{L}$.
Then, we have by associativity that
$\mathbf{M} = \left( \mathbf{L}^{-1} \mathbf{L} \right) \mathbf{M} =
\mathbf{L}^{-1} \left( \mathbf{L} \mathbf{M} \right) = \mathbf{L}^{-1}$.}
...
\end{pmatrix} \not\in \mathcal{S}_{+}^{2}
\end{equation}
so that we have $\mathbf{x}^{\operatorname{T}}\mathbf{P}\mathbf{x} = -1 < 0$
where $\mathbf{x} = \left( 1, 0 \right)^{\operatorname{T}}$.}

To see the first part~(1), we use mathematical induction.
It is trivial to show the case where $D = 1$.
For general $D > 1$, let us write $\mathbf{L}, \mathbf{M} \in \mathcal{T}_{+}^{D}$
as partitioned matrices so that
\begin{equation}
\mathbf{L} =
\begin{pmatrix}
\widehat{\mathbf{L}} & \mathbf{0} \\
\bm{\beta}^{\operatorname{T}} & \alpha
\end{pmatrix}, \qquad
\mathbf{M} =
\begin{pmatrix}
\widehat{\mathbf{M}} & \mathbf{0} \\
\bm{\eta}^{\operatorname{T}} & \gamma
\end{pmatrix}
\label{eq:partitioned_matrices_for_proof_of_triangular_matrix_group}
\end{equation}
where $\widehat{\mathbf{L}}, \widehat{\mathbf{M}} \in \mathcal{T}_{+}^{D - 1}$ and
$\alpha, \gamma > 0$.
Assuming that the product of $\widehat{\mathbf{L}}$ and $\widehat{\mathbf{M}}$ is again
in the group~$\mathcal{T}_{+}^{D - 1}$ so that
$\widehat{\mathbf{L}}\widehat{\mathbf{M}} \in \mathcal{T}_{+}^{D - 1}$,
we have
\begin{equation}
\mathbf{L}\mathbf{M} =
\begin{pmatrix}
...
=
\begin{pmatrix}
\widehat{\mathbf{L}}\widehat{\mathbf{M}} & \mathbf{0} \\
\bm{\lambda}^{\operatorname{T}} & \alpha\gamma
\end{pmatrix} \in \mathcal{T}_{+}^{D}
\end{equation}
where
\begin{equation}
\bm{\lambda} = \widehat{\mathbf{M}}^{\operatorname{T}}\bm{\beta} + \bm{\eta}\alpha .
\end{equation}

The second part~(2) can be shown similarly by induction.
Again, it is trivial to show the case where $D = 1$; and,
for general $D > 1$, we make use of partitioned matrices.
Assuming that the inverse~$\widehat{\mathbf{L}}^{-1}$ of
$\widehat{\mathbf{L}} \in \mathcal{T}_{+}^{D - 1}$ is in the group~$\mathcal{T}_{+}^{D - 1}$
so that $\widehat{\mathbf{L}}^{-1} \in \mathcal{T}_{+}^{D - 1}$,
we easily find the inverse~$\mathbf{L}^{-1}$ of $\mathbf{L} \in \mathcal{T}_{+}^{D}$ in the form
\begin{equation}
\mathbf{L}^{-1} =
\begin{pmatrix}
\widehat{\mathbf{L}} & \mathbf{0} \\
...
\end{pmatrix}^{-1} =
\begin{pmatrix}
\widehat{\mathbf{L}}^{-1} & \mathbf{0} \\
\bm{\xi}^{\operatorname{T}} & \alpha^{-1}
\end{pmatrix} \in \mathcal{T}_{+}^{D}
\end{equation}
where
\begin{equation}
\bm{\xi} = -\widehat{\mathbf{L}}^{-\operatorname{T}} \bm{\beta} \alpha^{-1} .
\end{equation}

\parhead{Cholesky decomposition}
For any symmetric positive-definite matrix~$\mathbf{A} \in \mathcal{S}_{+}^{D}$,
there exists a unique lower triangular matrix~$\mathbf{L} \in \mathcal{T}_{+}^{D}$
such that
\begin{equation}
\mathbf{A} = \mathbf{L} \mathbf{L}^{\operatorname{T}}
\label{eq:cholesky_decomposition}
\end{equation}
or, if the matrices~$\mathbf{A} = \left(A_{ij}\right)$ and $\mathbf{L} = \left(L_{ij}\right)$ are
...
The lower triangular matrix~$\mathbf{L}$ is called the \emph{Cholesky factor}.

Conversely, for any lower triangular matrix~$\mathbf{L} \in \mathcal{T}_{+}^{D}$,
...
function~$\mathcal{S}_{+}^{D} \to \mathcal{T}_{+}^{D}$.
...
indeed agrees with that of $\mathbf{L} \in \mathcal{T}_{+}^{D}$.
More specifically, either of $\mathbf{A}$ and $\mathbf{L}$ has $D(D + 1)/2$ independent parameters
because $\mathbf{A}$ is a symmetric matrix and $\mathbf{L}$ is a (lower) triangular matrix.
...
$L = \sqrt{A} > 0$ satisfies \eqref{eq:cholesky_decomposition}.
...
let us write $\mathbf{A} \in \mathcal{S}_{+}^{D}$ and $\mathbf{L} \in \mathcal{T}_{+}^{D}$
as partitioned matrices so that
\begin{equation}
\mathbf{A} =
\begin{pmatrix}
...
\begin{equation}
\mathbf{L}\mathbf{L}^{\operatorname{T}} =
\begin{pmatrix}
\widehat{\mathbf{L}} & \mathbf{0} \\
\bm{\beta}^{\operatorname{T}} & \alpha
\end{pmatrix}
\begin{pmatrix}
\widehat{\mathbf{L}}^{\operatorname{T}} & \bm{\beta} \\
\mathbf{0} & \alpha
\end{pmatrix}
=
\begin{pmatrix}
\widehat{\mathbf{L}}\widehat{\mathbf{L}}^{\operatorname{T}} & \widehat{\mathbf{L}}\bm{\beta} \\
\left( \widehat{\mathbf{L}}\bm{\beta} \right)^{\operatorname{T}} & \|\bm{\beta}\|^{2} + \alpha^{2}
...
gives
\begin{align}
\widehat{\mathbf{A}} &= \widehat{\mathbf{L}} \widehat{\mathbf{L}}^{\operatorname{T}}
\label{eq:cholesky_decomposition_D-1} \\
\mathbf{b} &= \widehat{\mathbf{L}}\bm{\beta} \\
a &= \|\bm{\beta}\|^{2} + \alpha^{2} .
\end{align}
We see that \eqref{eq:cholesky_decomposition_D-1} is
the Cholesky decomposition from $\widehat{\mathbf{A}} \in \mathcal{S}_{+}^{D - 1}$
to $\widehat{\mathbf{L}} \in \mathcal{T}_{+}^{D - 1}$.
Assuming that this $(D - 1)$-dimensional
Cholesky decomposition~\eqref{eq:cholesky_decomposition_D-1} exists,
we can find $\bm{\beta}$ and $\alpha$ as
\begin{align}
\bm{\beta} &= \widehat{\mathbf{L}}^{-1} \mathbf{b} \label{eq:beta_in_cholesky_factor} \\
\alpha &= \sqrt{a - \|\bm{\beta}\|^{2}} > 0 \label{eq:alpha_in_cholesky_factor} .
\end{align}
Note that we always have \eqref{eq:alpha_in_cholesky_factor} because $a - \|\bm{\beta}\|^{2} > 0$,
...
This can also be shown by noting that
...
$\mathbf{L} \in \mathcal{T}_{+}^{D}$ from $\mathbf{A} \in \mathcal{S}_{+}^{D}$.
...
Bijectivity of the Cholesky decomposition~\eqref{eq:cholesky_decomposition} allows us to use
the matrix transformation as a change of variables,
of which we shall later make use for evaluating certain integrals over
symmetric positive-definite matrices.
Here, we consider Jacobians for such matrix transformations.

First, let us find the Jacobian matrix for
the Cholesky decomposition~\eqref{eq:cholesky_decomposition} between
$\mathbf{A} \in \mathcal{S}_{+}^{D}$ and $\mathbf{L} \in \mathcal{T}_{+}^{D}$.
...
we only consider the lower triangular elements~$A_{ij}$ of $\mathbf{A}$ such that $i \geqslant j$.
Writing down the lower triangular elements~$A_{ij}$ in \emph{column-major} order,
i.e., $\left( A_{11}, A_{21}, \dots, A_{D1}, A_{22}, \dots, A_{D2}, \dots, A_{DD} \right)$,
we have
\begin{align}
A_{11} &= L_{11}^{2} \\
A_{21} &= L_{21} L_{11} \\
...
\shortvdotswithin{=}
A_{D2} &= L_{D1} L_{21} + L_{D2} L_{22} \\
\shortvdotswithin{=}
A_{ij} &= L_{i1} L_{j1} + L_{i2} L_{j2} + \dots + L_{ij} L_{jj} \quad (i \geqslant j) \\
\shortvdotswithin{=}
A_{DD} &= L_{D1}^{2} + L_{D2}^{2} + \dots + L_{DD}^{2} .
\end{align}
Since $A_{ij}$ depends only on $L_{ij}$ and
the preceding elements of $\mathbf{L}$ in column-major order,
the Jacobian matrix from $\mathbf{L}$ to $\mathbf{A}$ is a lower triangular matrix of the form
\begin{align}
\frac{\partial\mathbf{A}}{\partial\mathbf{L}}
&\equiv
\frac{\partial \left( A_{11}, A_{21}, \dots, A_{D1}, A_{22}, \dots, A_{D2}, \dots, A_{DD} \right)}
{\partial \left( L_{11}, L_{21}, \dots, L_{D1}, L_{22}, \dots, L_{D2}, \dots, L_{DD} \right)} \\
&=
\bordermatrix{
         &   L_{11} & L_{21} & \cdots & L_{D1} &   L_{22} & \cdots & L_{D2} & \cdots &   L_{DD} \cr
  A_{11} & 2 L_{11} & 0      & \cdots & 0      & 0        & \cdots & 0      & \cdots & 0        \cr
  A_{21} & \ast     & L_{11} & \cdots & 0      & 0        & \cdots & 0      & \cdots & 0        \cr
\;\vdots & \vdots   & \vdots & \ddots & \vdots & \vdots   & \ddots & \vdots & \ddots & \vdots   \cr
  A_{D1} & \ast     & \ast   & \cdots & L_{11} & 0        & \cdots & 0      & \cdots & 0        \cr
  A_{22} & \ast     & \ast   & \cdots & \ast   & 2 L_{22} & \cdots & 0      & \cdots & 0        \cr
\;\vdots & \vdots   & \vdots & \ddots & \vdots & \vdots   & \ddots & \vdots & \ddots & \vdots   \cr
  A_{D2} & \ast     & \ast   & \cdots & \ast   & \ast     & \cdots & L_{22} & \cdots & 0        \cr
\;\vdots & \vdots   & \vdots & \ddots & \vdots & \vdots   & \ddots & \vdots & \ddots & \vdots   \cr
  A_{DD} & \ast     & \ast   & \cdots & \ast   & \ast     & \cdots & \ast   & \cdots & 2 L_{DD}
}
\in \mathcal{T}_{+}^{D(D + 1)/2}
\end{align}
where the diagonal elements are given by
\begin{equation}
\frac{\partial A_{jj}}{\partial L_{jj}} = 2 L_{jj} > 0, \qquad
\frac{\partial A_{ij}}{\partial L_{ij}} = L_{jj} > 0 \quad (i > j)
\end{equation}
and the elements denoted by $\ast$ are elements possibly nonzero.
Thus, we have
\begin{equation}
\left| \frac{\partial\mathbf{A}}{\partial\mathbf{L}} \right| =
2 L_{11}^{D} \times 2 L_{22}^{D - 1} \times \dots \times 2 L_{DD} =
2^D \prod_{i=1}^{D} L_{ii}^{D + 1 - i} .
\end{equation}
...
\parhead{Matrix derivatives identities}
