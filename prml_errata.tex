...
In fact, the change of variables based on the Cholesky decomposition is
useful for evaluating certain integrals over symmetric positive-definite matrices
...
Writing down the upper triangular elements~$A_{ij}$ in \emph{row-major} order,
i.e., $\left( A_{11}, A_{12}, \dots, A_{1D}, A_{22}, \dots, A_{2D}, \dots, A_{DD} \right)$,
we have
\begin{align}
A_{11} &= U_{11}^{2} \\
A_{12} &= U_{11} U_{12} \\
...
\end{align}
Since $A_{ij}$ depends only on $U_{ij}$ and its preceding elements of $\mathbf{U}$,
the Jacobian matrix from $\mathbf{U}$ to $\mathbf{A}$ is a lower triangular matrix of the form
\begin{align}
...
Another matrix transformation of interest is the product of upper triangular matrices
\begin{equation}
\mathbf{U} = \mathbf{R} \mathbf{G}
\end{equation}
where $\mathbf{U}, \mathbf{R}, \mathbf{G} \in \mathcal{U}_{+}^{D}$.
Assuming that $\mathbf{G}$ is constant, we see the above product as a matrix transformation
between $\mathbf{U}$ and $\mathbf{R}$.
If the matrices~$\mathbf{U} = \left(U_{ij}\right)$, $\mathbf{R} = \left(R_{ij}\right)$,
and $\mathbf{G} = \left(G_{ij}\right)$ are written element-wise, we have
\begin{equation}
\begin{pmatrix}
U_{11} & U_{12} & \cdots & U_{1D} \\
0      & U_{22} & \cdots & U_{2D} \\
...
U_{DD} &= R_{DD} G_{DD} .
\end{align}
Since $U_{ij}$ depends only on $R_{ij}$ and its preceding elements,
the Jacobian matrix from $\mathbf{R}$ to $\mathbf{U}$ is again a lower triangular matrix
where the diagonal elements are given by
\begin{equation}
\frac{\partial U_{ij}}{\partial R_{ij}} = G_{jj} > 0 \quad (i \leqslant j) .
\end{equation}
Thus, we obtain
\begin{equation}
\left| \frac{\partial\mathbf{U}}{\partial\mathbf{R}} \right| =
G_{11} \times G_{22} \times \dots \times G_{DD} \times
G_{22} \times \dots \times G_{DD} \times \dots \times
G_{DD} =
\prod_{i=1}^{D} G_{ii}^{\,i} .
\end{equation}
