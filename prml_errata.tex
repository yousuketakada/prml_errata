...
% TODO: use paragraph headers to better organize the text
\newcommand{\parhead}[1]{%
\paragraph{#1}
...
%\parhead{Some properties of the log minus digamma function.}
...
that we apply the EM algorithm to maximum likelihood for
...
maximum likelihood for the gamma distribution~(2.146) here,
...
\parhead{Maximum likelihood for the gamma distribution.}
...
Next we maximize \eqref{eq:log_likelihood_for_gamma_in_terms_of_a} with respect to $a$.
This can be done by setting the derivative of \eqref{eq:log_likelihood_for_gamma_in_terms_of_a}
with respect to $a$ equal to zero, giving a nonlinear equation of the form
\begin{equation}
\varphi(a) = \ln\overline{x} - \ln\widehat{x}
\label{eq:nonlinear_equation_for_a_ML}
\end{equation}
where $\varphi(\cdot)$ is the log minus digamma function given by
\eqref{eq:log_minus_digamma_function}.
One can see that \eqref{eq:log_likelihood_for_gamma_in_terms_of_a} is
a strictly concave function of $a$
because $\varphi(\cdot)$ is a strictly monotonically decreasing function
so that $\varphi'(a) < 0$ for all $a > 0$.
...
and thus has its inverse~$\varphi^{-1}: (0, \infty) \to (0, \infty)$,
we can solve \eqref{eq:nonlinear_equation_for_a_ML} uniquely for $a > 0$,
which gives the maximum likelihood solution for $a$
\begin{equation}
a_{\text{ML}} = \varphi^{-1}(\ln\overline{x} - \ln\widehat{x}) .
\end{equation}
Substituting this into $b = a/\overline{x}$,
we finally obtain the maximum likelihood solution for $b$
\begin{equation}
