...
because otherwise we do not have \eqref{eq:differentiating_integral_identity_wrt_parameters}.
Let us take
...
as an example.
Since we cannot change a parameter~$\mu_k$
(the normalized probability of observing component~$k$)
...
Instead, we should consider the derivatives with respect to independent parameters, say,
unnormalized probabilities~$\widetilde{\mu}_k$
that are related to $\mu_k$ through \eqref{eq:normalized_mu_from_unnormalized_mu}.
...
useful for remembering \eqref{eq:gradient_of_Tr_AB_wrt_A}:
The gradient of a scalar with respect to a matrix~$\mathbf{A}$ is,
...
meaning that the operations in \eqref{eq:gradient_of_Tr_AB_wrt_A} are meaningful.
...
identifying $x$ with $A_{ij}$ in \eqref{eq:partial_derivative_of_log_abs_det} and
...
\end{equation}
If we adopt the notation~\eqref{eq:abs_of_matrix_determinant} for $\left|\mathbf{A}\right|$,
...
We finally have \eqref{eq:gradient_of_Tr_A_B_Atranspose_C} by
