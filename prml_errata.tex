...
\erratum{Pages~5 and 10}
Equation~(1.1) and the text after (1.4):
The ellipsis should be centered, i.e., ``$\cdots$''
(\texttt{\char`\\cdots} or, with the \texttt{amsmath} or \texttt{mathtools} package,
\texttt{\char`\\dots} will do)
instead of ``$\ldots$'' (\texttt{\char`\\ldots}).
For example, (1.1) should read
\begin{equation}
...
Line~\textminus1:
The ellipsis should be centered, i.e., ``$\cdots$''
(\texttt{\char`\\cdots} or \texttt{\char`\\dots} will do).
...
However, it is useful to note that, at the convergence of IRLS,
we have the following implicit condition for $\mathbf{w}^{\star}$
\begin{equation}
...
making use of \eqref{eq:implicit_condition_for_posterior_mode_of_classification_RVM},
...
a categorical (or multinoulli) distribution~\eqref{eq:categorical_distribution} so that
\begin{equation}
...
\erratum{Page~504}
Line~1:
Here, we should use the general result~\eqref{eq:optimal_solution_for_local_variational_bayes}
for the optimal solution for local variational Bayes.
Note that the result~\eqref{eq:optimal_solution_for_local_variational_bayes} is slightly different
from the general result \eqref{eq:optimal_solution_for_variational_bayes}, or (10.9), for
...
``our predictions for $\mathbf{x}_{n + 1}$ depend on\dots'' (Remove \emph{s} from the verb).
...
and the covariance.
Specifically, (13.104) and the preceding text should read, e.g.,
...
\erratum{Page~690}
Equation~(B.54):
The discrete distribution of the form~(B.54), or (2.26), is sometimes called
the \emph{categorical} or \emph{multinoulli} distribution.
It is a generalization of the Bernoulli distribution~(B.1) to a $K$-dimensional binary
variable~$\mathbf{x} = (x_1, \dots, x_K)^{\operatorname{T}}$
where $x_k \in \{0, 1\}$ such that $\sum_k x_k = 1$, i.e.,
we use the one-of-$K$ coding scheme for $\mathbf{x}$;
